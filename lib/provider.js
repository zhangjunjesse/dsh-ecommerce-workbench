...
function createToapisProvider(options) {
   ...
   * @param {{images?: Array<ImageInput>, prompt?: string, count?: number, size?: string}} input
   *   `size` is the output ratio ("3:4", "1:1", …). Omitted means the service
   *   default (1:1) — the daily-driver surface is left alone, and the steps that
   *   know their shape (换装+裂变 works from 3:4 scene photos) pass it explicitly.
   * @returns {Promise<ImageOutput[]>}
   */
  async function generate(input) {
    ...
    try {
      const args = ["--mode", images.length > 0 ? "edit" : "generate", "--prompt", prompt];
      if (typeof input.size === "string" && input.size !== "") args.push("--size", input.size);
  ...
  async function applyToTshirt(input) {
    ...
    try {
      const args = ["--mode", "edit", "--prompt", applyToTshirtPrompt(input.prompt)];
      // Order is load-bearing, and the prompt is written against it: **the garment's
      // own photos come first and the 印花 is appended LAST**, so the prompt can say
      // "前面几张是这件衣服…最后一张是要印上去的图案".
      //
      // Leading with the 印花 also works — both orders were measured on a real 款式
      // (6 references) and both picked the front and replaced the garment's existing
      // print correctly. Leading with the garment wins because the composition then
      // anchors on the real article rather than on the artwork.
      const refs = Array.isArray(input.refs) ? input.refs : [];
      let refCount = 0;
      for (const ref of refs) {
        if (!ref || !ref.buffer) continue;
        const refPath = join(dir, "ref" + refCount + extFor(ref.mimeType || "image/png"));
        await writeFile(refPath, ref.buffer);
        args.push("--image", refPath);
        refCount++;
      }
      const printPath = join(dir, "print" + extFor(input.printMimeType));
      await writeFile(printPath, input.printBuffer);
      args.push("--image", printPath);
      const outBase = join(dir, "out");
      // 3:4, because the source photos are 3:4 and the prompt asks for a product
      // shot. The service defaults to 1:1, and a square reply is not a crop of the
      // source — the composition was planned for a square frame, so the garment
      // arrives re-arranged and cannot be cropped back.
      args.push("--size", "3:4", "--output", outBase, "--output-dir", dir, "--n", String(count));
      const result = await run(args);
      const outputs = [];
      for (const file of result.files || []) {
        outputs.push({ buffer: await readFile(file), mimeType: mimeFor(file) });
