...
const { effectiveSettings } = require("./workflows.js");
...
/** Run params are carried into the record and the history file; keep them small. */
const MAX_PARAMS_CHARS = 4096;
...
/**
 * Keep run params to something small, plain and JSON-safe.
 *
 * They are persisted with the run and handed back to the workflow, so anything
 * unserializable (a function, a cycle) or huge has to be rejected here rather
 * than silently corrupting the run history file later.
 */
function normalizeParams(params) {
  ...
  if (typeof encoded !== "string") return null; // undefined, functions
  if (encoded.length > MAX_PARAMS_CHARS) {
    throw Object.assign(new Error("workflow params are too large (" + encoded.length + " chars)"), { code: "BAD_PARAMS" });
  }
...
function createWorkflowRunner(options) {
  ...
  function buildContext(record, settings) {
    return {
      runId: record.id,
      ...
      // The user's saved values, clamped to the declared ranges. Resolved once, at
      // the start, so a Setting changed mid-run cannot make one run work with two
      // different shapes.
      settings: settings,
  ...
  /** The effective settings for a workflow, read from the store at run start. */
  async function resolveSettings(definition) {
    if (!definition.settings || definition.settings.length === 0) return {};
    try {
      const state = await store.read();
      const config = (state.workflows || []).filter(function (w) { return w.id === definition.id; })[0];
      return effectiveSettings(definition, config ? config.settings : null);
    } catch (error) {
      // Unreadable state must not stop the run; the declared defaults are a
      // perfectly good answer, and they are what a fresh install would use.
      console.warn("[ecommerce-workbench] falling back to default workflow settings: " + text(error));
      return effectiveSettings(definition, null);
    }
  }
  ...
  async function execute(record, definition) {
    ...
    try {
      const settings = await resolveSettings(definition);
      const summary = await definition.run(buildContext(record, settings));
   ...
   *
   * `params` is whatever the caller wants the workflow to act on this time (a
   * group to process, a "force" flag). The engine deliberately does not interpret
   * it — deciding *what* to work on is the workflow's own business — it only
   * carries it into `ctx.params` and records it, so a run stays explainable after
   * the fact.
   */
  function start(workflowId, trigger, params) {
  ...
  async function run(workflowId, trigger, params) {
    const record = start(workflowId, trigger, params);
