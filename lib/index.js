 ...
 */
const { createStore, normalizeTshirt } = require("./store.js");
...
function createHandler(store, provider, options) {
  ...
  async function pipelineGroups() {
    ...
    return scanned.map(function (group) {
      ...
      return {
        ...
        // The 款式 selection. `tshirtImages` stays on the record for groups that
        // predate 款式; the picker reads this one.
        tshirtColorways: Array.isArray(config.tshirtColorways) ? config.tshirtColorways : null,
  ...
  async function handler(req, res) {
    ...
    try {
      ...
      // One reference image out of a group's inbox folder.
      //
      // A group's screenshots are read straight off disk by the pipeline and are
      // never copied into the store, so `/ecom/api/file/` cannot serve them — the
      // names in `group.images` are the uploaded file names, not store ids. The UI
      // still has to show them: a group is called 「组3」, and "3 张参考图" is not an
      // identity, because screenshots of three different products look identical
      // until you can see them.
      if (req.method === "GET" && path === "/ecom/api/workflow/group/image") {
        const groupKey = url.searchParams.get("groupKey") || "";
        const name = url.searchParams.get("name") || "";
        // Both halves come from the client, so both are checked before either
        // reaches the filesystem: the key cannot climb out of the inbox root, and
        // the file name cannot climb out of the group's own folder.
        if (name === "" || name.includes("/") || name.includes("\\") || name.includes("..")) {
          return writeJson(res, 400, { ok: false, error: "invalid name" });
        }
        let dir;
        try {
          dir = groupDirectory(store, normalizeGroupKey(groupKey));
        } catch (error) {
          return writeJson(res, 400, { ok: false, error: String((error && error.message) || error) });
        }
        try {
          const buffer = await readFile(joinPath(dir, name));
      ...
      // T恤管理: one product grouped by 款式 (colour). Each 款式 owns its 白底图
      // (front, back, …) and its 细节图 (collar, cuff, fabric); 尺码图 hang off the
      // product, because one pattern means one size chart. See store.normalizeTshirt
      // for the canonical shape, for how the flat `images` field is derived from
      // it, and for how a record written before 款式 existed is read.
      if (path === "/ecom/api/tshirt/create") {
        ...
        // The first upload IS the first 款式: creating a T恤 means "here are its
        // photos", so a separate "now add a colour" step would be a step for
        // nothing. Everything lands in 白底图 — a detail shot can be moved by
        // deleting and re-adding it under the right slot.
        const tshirt = {
          id: recordId(),
          name: typeof payload.name === "string" && payload.name.trim() ? payload.name.trim() : "未命名T恤",
          colorways: [{
            id: recordId(),
            name: typeof payload.colorwayName === "string" && payload.colorwayName.trim() ? payload.colorwayName.trim() : "款式 1",
            white: files,
            detail: []
          }],
          sizeImages: [],
        ...
        // Normalized on the way out as well as on the way in: `images` is derived,
        // and the client renders this reply directly rather than re-reading state,
        // so a record sent without it would render as a T恤 with no photos.
        return writeJson(res, 200, { ok: true, tshirt: normalizeTshirt(tshirt) });
      }

      // Add a 款式 (colour) to an existing T恤. It starts empty; its photos arrive
      // through addImages like any other bucket.
      if (path === "/ecom/api/tshirt/addColorway") {
        let updated = null;
        await store.update(function (state) {
          state.tshirts = state.tshirts.map(function (t) {
            ...
            const name = typeof payload.name === "string" && payload.name.trim()
              ? payload.name.trim()
              : "款式 " + (t.colorways.length + 1);
            updated = Object.assign({}, t, {
              colorways: t.colorways.concat([{ id: recordId(), name: name, white: [], detail: [] }])
            });
            return updated;
          });
        });
        if (updated === null) return writeJson(res, 404, { ok: false, error: "tshirt not found" });
        return writeJson(res, 200, { ok: true, tshirt: normalizeTshirt(updated) });
      }

      // Rename the product, or one of its 款式 when colorwayId is given.
      if (path === "/ecom/api/tshirt/rename") {
        const next = typeof payload.name === "string" ? payload.name.trim() : "";
        if (next === "") return writeJson(res, 400, { ok: false, error: "name required" });
        const colorwayId = typeof payload.colorwayId === "string" && payload.colorwayId !== "" ? payload.colorwayId : null;
        let updated = null;
        await store.update(function (state) {
          state.tshirts = state.tshirts.map(function (t) {
            if (t.id !== payload.id) return t;
            if (colorwayId === null) {
              updated = Object.assign({}, t, { name: next });
            } else {
              updated = Object.assign({}, t, {
                colorways: t.colorways.map(function (c) { return c.id === colorwayId ? Object.assign({}, c, { name: next }) : c; })
              });
            }
            return updated;
          });
      ...
      // Add photos to ONE bucket: a 款式's 白底图 / 细节图, or the product's 尺码图.
      // A missing `slot` means 白底图 and a missing colorwayId means the first
      // 款式, so a caller that predates 款式 keeps doing exactly what it did.
      if (path === "/ecom/api/tshirt/addImages") {
        const images = Array.isArray(payload.images) ? payload.images : [];
        if (images.length === 0) return writeJson(res, 400, { ok: false, error: "images required" });
        const SLOTS = { white: "white", detail: "detail", model: "model", size: "sizeImages" };
        const slot = payload.slot === undefined ? "white" : SLOTS[payload.slot];
        if (slot === undefined) return writeJson(res, 400, { ok: false, error: "slot must be white, detail, model or size" });
        const files = [];
        for (const image of images) {
          const decoded = decodeDataUrl(image.dataUrl);
          files.push(await store.putFile(decoded.buffer, decoded.mimeType));
        }
        const wantedColorway = typeof payload.colorwayId === "string" && payload.colorwayId !== "" ? payload.colorwayId : null;
        let updated = null;
        await store.update(function (state) {
          state.tshirts = state.tshirts.map(function (t) {
            if (t.id !== payload.id) return t;
            if (slot === "sizeImages") {
              updated = Object.assign({}, t, { sizeImages: t.sizeImages.concat(files) });
              return updated;
            }
            const target = wantedColorway === null ? t.colorways[0] : t.colorways.filter(function (c) { return c.id === wantedColorway; })[0];
            // An unknown 款式 is reported as "not found" rather than silently
            // landing on the first one: the photos are already on disk, so a
            // wrong-but-successful reply would file them under the wrong colour.
            if (target === undefined) return t;
            updated = Object.assign({}, t, {
              colorways: t.colorways.map(function (c) {
                if (c.id !== target.id) return c;
                const bucket = Object.assign({}, c);
                bucket[slot] = c[slot].concat(files);
                return bucket;
              })
            });
            return updated;
        ...
        if (updated === null) return writeJson(res, 404, { ok: false, error: "tshirt or 款式 not found" });
        return writeJson(res, 200, { ok: true, tshirt: normalizeTshirt(updated) });
      }
        ...
        const orphans = await store.update(function (state) {
          ...
          if (payload.kind === "library") {
            ...
            state.tshirts = state.tshirts.filter(function (t) {
              ...
              // Every bucket takes its bytes with it — each 款式's 白底图 and 细节图,
              // and the product's 尺码图. Missing one leaks a file no record points
              // at, which is exactly what the orphan sweep exists to prevent.
              t.colorways.forEach(function (c) {
                c.white.forEach(function (f) { files.push(f); });
                c.detail.forEach(function (f) { files.push(f); });
                c.model.forEach(function (f) { files.push(f); });
              });
              t.sizeImages.forEach(function (f) { files.push(f); });
              return false;
            });
          } else if (payload.kind === "tshirtColorway") {
            state.tshirts = state.tshirts.map(function (t) {
              if (t.id !== payload.id) return t;
              const kept = t.colorways.filter(function (c) {
                if (c.id !== payload.colorwayId) return true;
                c.white.forEach(function (f) { files.push(f); });
                c.detail.forEach(function (f) { files.push(f); });
                c.model.forEach(function (f) { files.push(f); });
                return false;
              });
              return Object.assign({}, t, { colorways: kept });
            });
          } else if (payload.kind === "tshirtImage") {
            // The file can sit in any bucket — a 款式's 白底图 or 细节图, or the
            // product's 尺码图 — and the caller knows only the file name. Searching
            // every bucket is what lets one delete path serve all of them, and it
            // is why 细节图 need no separate removal route.
            ...
            state.tshirts = state.tshirts.map(function (t) {
              ...
              return Object.assign({}, t, {
                colorways: t.colorways.map(function (c) {
                  return Object.assign({}, c, { white: drop(c.white), detail: drop(c.detail), model: drop(c.model) });
                }),
                sizeImages: drop(t.sizeImages)
              });
            });
      ...
      if (path === "/ecom/api/clear") {
        const orphans = await store.update(function (state) {
          ...
          if (payload.kind === "library") {
            ...
            state.tshirts.forEach(function (t) {
              t.colorways.forEach(function (c) {
                c.white.forEach(function (f) { files.push(f); });
                c.detail.forEach(function (f) { files.push(f); });
                c.model.forEach(function (f) { files.push(f); });
      ...
      if (path === "/ecom/api/workflow/group/approve") {
        ...
        // 款式 selection, stored next to the older per-photo list rather than
        // replacing it: a group configured before 款式 existed still carries
        // `tshirtImages`, and resolveTshirt reads that as "the 款式 those photos
        // belong to" — so an old group keeps running against what it always did.
        if (Array.isArray(payload.tshirtColorways)) {
          patch.tshirtColorways = payload.tshirtColorways.filter(function (id) { return typeof id === "string" && id !== ""; });
        }
