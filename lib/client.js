...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      var JOB_PATHS = { extract: "/extract", recreate: "/recreate", tshirtRecreate: "/tshirtRecreate" };
      ...
      //
      // Re-bind ONLY the area hosting this workbench back to a shrinkable,
      // definite height. `:has(> .dsh-ecom-root)` selects that ancestor by
      // structure, so this does not depend on the shell's hashed class names
      ...
      function injectLayoutStyle() {
        ...
        el.textContent =
          "div:has(> ." + ECOM_ROOT_CLASS + "){flex:1 1 0!important;min-height:0!important;overflow:hidden!important;}" +
          "." + ECOM_ROOT_CLASS + "{height:100%;min-height:0;}";
      ...
      function Workbench() {
        ...
        // Fallback for engines without `:has()`: bind the hosting view area
        // directly, so the layout fix does not depend on that selector.
        var rootRef = React.useRef(null);
        React.useEffect(function () {
          var hasSupported = typeof CSS !== "undefined" && CSS.supports && CSS.supports("selector(:has(*))");
          if (hasSupported) return undefined;
          var parent = rootRef.current && rootRef.current.parentElement;
          if (!parent) return undefined;
          var prev = { flex: parent.style.flex, minHeight: parent.style.minHeight, overflow: parent.style.overflow };
          parent.style.flex = "1 1 0";
          parent.style.minHeight = "0";
          parent.style.overflow = "hidden";
          return function () {
            parent.style.flex = prev.flex;
            parent.style.minHeight = prev.minHeight;
        ...
        var secondaryNav = [["T恤管理", IconShirt, true], ["提示词管理", IconBookmark, true]].map(navButton);
        ...
        var viewNames = ["印花提取", "印花二创", "T恤二创", "T恤管理", "提示词管理"];
