...
// T恤's reference photos), 通用工作台 (free-form prompt + references), 提示词管理
// (saved prompts) and 场景图管理 (paste-and-store scene photos, no generation).
  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      function Workbench() {
        ...
        var viewNames = ["印花提取", "印花二创", "T恤二创", "通用工作台", "T恤管理", "提示词管理", "场景图管理"];
      ...
      function SceneManager(props) {
        ...
        var toolbar = scenes.length > 0 ? h("div", { style: { flex: "0 0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", margin: "2px 2px 12px" } },
          h("span", { style: { ...UI.muted } }, scenes.length + " 张场景图"),
          h("button", { style: Object.assign({}, UI.btnGhost, { display: "flex", alignItems: "center", gap: 5 }), onClick: clearAll }, IconTrash(13), "清空")) : null;

        // Waterfall: the scroll container and the multi-column box are separate
        // elements on purpose. Handing `column-width` a container with a definite
        // height would let an over-long pool flow into extra columns to the right
        // (horizontal overflow); letting the column box size itself to its own
        // content keeps the columns vertical and the outer box the only scroller.
        var pool;
        if (scenes.length > 0) {
          pool = h("div", { style: { flex: 1, minHeight: 0, overflow: "auto", padding: "2px 4px 16px" } },
            h("div", { style: { columnWidth: 220, columnGap: 12 } },
              scenes.slice().sort(byNewest).map(function (s) {
                var src = fileUrl(s.file);
                return h("div", {
                  key: s.id,
                  style: { position: "relative", breakInside: "avoid", WebkitColumnBreakInside: "avoid", marginBottom: 12, borderRadius: 12, overflow: "hidden", border: "1px solid #ececf0", background: "#ffffff" }
                },
                  h("img", {
          ...
          pool = h("div", { style: { flex: 1, minHeight: 0, display: "flex", alignItems: "center", justifyContent: "center" } },
            h("div", { style: { ...UI.muted, textAlign: "center" } }, "暂无内容"));
        }
