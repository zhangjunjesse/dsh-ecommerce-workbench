...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      /**
       * One 款式 (SKU) in the 成品库 feed: **a single image**, with the rest of
       * that 款式's shots reachable by dragging it left/right or with the arrows.
       *
       * Showing one image per 款式 rather than every shot is the whole point of a
       * shopping feed: a 商品 with 24 款式 would otherwise be 192 near-identical
       * tiles, and the eye would have nothing to compare. One card per 款式 makes
       * the shelf a list of *things*, and the shots of a thing belong behind a
       * swipe — exactly like a product listing where you swipe the photo.
       *
       * The card keeps a fixed aspect ratio (the first shot's measured one), so
       * swiping never changes the card's height and the grid cannot jump. The
       * shots are 3:4 by construction (the 换装+裂变 prompt asks for 3:4竖版), so
       * covering that box crops nothing in practice; anything genuinely off-ratio
       * is shown whole on the page this card opens.
       */
      function ProductCard(props) {
        ...
        return h("div", { style: { borderRadius: 12, overflow: "hidden", border: "1px solid #ececf0", background: "#ffffff" } },
          h("div", {
            style: { position: "relative", background: "#f2f2f4", touchAction: "pan-y" },
            ...
            h("img", {
              ...
              style: {
                width: "100%", display: "block", aspectRatio: card.aspect || "3 / 4",
                objectFit: "cover", cursor: "pointer", background: "#f2f2f4"
              }
            }),
          ...
          h("div", {
            title: card.groupName + " · 款式 #" + (card.styleIndex + 1) + " · " + shots.length + " 张成片",
            // The caption opens the page too: expecting the user to hit the
            // image exactly is a needlessly small target.
            onClick: open,
            style: { display: "flex", alignItems: "center", gap: 5, padding: "5px 7px", cursor: "pointer" }
          },
            h("span", { style: { flex: 1, minWidth: 0, color: UI.text, fontSize: 11.5, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, card.groupName),
            h("span", { style: { flex: "0 0 auto", color: "#a9acb3", fontSize: 11 } }, "#" + (card.styleIndex + 1))));
      }
      ...
      function ProductLibrary(props) {
        ...
        goods.forEach(function (good) {
          ...
          good.styles.forEach(function (card, index) {
            ...
            // The card's box comes from its first shot's measured ratio; the rest
            // of the pass then swaps inside a box that never changes size.
      ...
      /**
       * One 商品, opened as a **modal** over the feed.
       *
       * Big cover in the middle with ‹ › either side, a rail of the same 款式's
       * other shots down the left, the 款式 (二创T恤) switcher below, and what the
       * current shot is made of on the right. Arrow keys move between shots;
      ...
      function ProductModal(props) {
        ...
        // The modal shows the one 款式 the feed opened, and nothing else: there is
        // deliberately no 款式 strip along the bottom. Switching 款式 is what the
        // shelf is for — pick another card — and the height that strip took now
        // belongs to the image, which is the thing actually being looked at.
        var safeStyleIndex = Math.min(Math.max(focusStyle, 0), Math.max(styles.length - 1, 0));
        ...
        return h("div", {
          ...
          h("div", {
            ...
            h("div", { style: { flex: 1, minHeight: 0, display: "flex", gap: 12 } },
              ...
              h("div", { style: { flex: "0 0 190px", minHeight: 0, display: "flex", flexDirection: "column", gap: 10, overflowY: "auto" } },
