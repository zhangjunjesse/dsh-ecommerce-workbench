  ...
  factory: function (require) {
    ...
    function apply(ctx) {
        ...
        return h("div", { style: { flex: "0 0 auto", background: "#ffffff", border: "1px solid #ececf0", borderRadius: 14, padding: "14px 16px", boxShadow: "0 1px 3px rgba(20,20,25,0.04)" } },
          h("div", { style: { display: "flex", alignItems: "flex-start", gap: 12 } },
            ...
            h("div", { style: { display: "flex", alignItems: "center", gap: 8, flex: "0 0 auto" } },
              ...
              // 印花流水线 is driven by a queue of reference groups, so its card
              // carries one more entry point than a plain workflow's.
              w.id === PIPELINE_WORKFLOW_ID ? h("button", {
                onClick: function () { props.onToggleGroups(); },
                style: Object.assign({}, smallBtn, props.groupsOpen ? { background: "#eceef1" } : null)
              }, "分组与成品") : null,
              h("button", {
      ...
      function PipelinePanel(props) {
        ...
        function upload(files) {
          ...
          Promise.all(list.map(readAsDataUrl)).then(function (dataUrls) {
            return apiPost("/workflow/group/upload", {
              name: uploadName.trim(),
              images: list.map(function (file, index) { return { name: file.name, dataUrl: dataUrls[index] }; })
            });
          }).then(function (res) {
            setBusy("");
        ...
        var uploadBar = h("div", { style: { flex: "0 0 auto", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 12, padding: "10px 12px", borderRadius: 12, background: "#f7f8fa", border: "1px solid #ececf0" } },
          h("input", {
            value: uploadName,
            onChange: function (e) { setUploadName(e.target.value); },
            placeholder: "分组名，例如「商品A」（同一个商品的截图放一组）",
            style: { flex: "1 1 240px", minWidth: 0, border: "1px solid #e5e5ea", outline: 0, borderRadius: 9, padding: "7px 10px", fontSize: 12.5, color: UI.text, background: "#fff", boxSizing: "border-box" }
          }),
          h("input", {
            ref: fileRef,
            type: "file",
            ...
            style: { flex: "0 0 auto", fontSize: 12.5, color: UI.text2 }
          }),
          busy === "upload" ? h("span", { style: { display: "flex", alignItems: "center", gap: 6, ...UI.muted } }, Spinner(), "正在写入…") : null,
          h("span", { style: { flex: "1 1 100%", color: "#a9acb3", fontSize: 11.5 } },
      ...
      function WorkflowManager(props) {
        ...
        // Which workflow's group/product panel is expanded (印花流水线 only).
        var groupsOpenState = React.useState(null);
        var groupsOpenId = groupsOpenState[0]; var setGroupsOpenId = groupsOpenState[1];
        ...
        return h("div", { style: { flex: 1, minHeight: 0, overflow: "hidden", display: "flex", flexDirection: "column" } },
          ...
          h("div", { style: { flex: 1, minHeight: 0, overflow: "auto", display: "flex", flexDirection: "column", gap: 12, padding: "2px 4px 16px" } },
            workflows.map(function (w) {
              ...
              var groupsOpen = groupsOpenId === w.id;
              return h(React.Fragment, { key: w.id },
                h(WorkflowCard, {
                  ...
                  onToggleGroups: function () { setGroupsOpenId(groupsOpen ? null : w.id); }
                }),
                groupsOpen ? h(PipelinePanel, {
                  workflows: workflows,
