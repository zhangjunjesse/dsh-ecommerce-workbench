  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      function PipelinePanel(props) {
        ...
        function estimateBlock(plan) {
          ...
          h("div", { style: { display: "flex", gap: 14, flexWrap: "wrap", fontSize: 12.5, color: UI.text2, marginBottom: 8 } },
            ...
            h("span", { key: "d" }, "T恤融合 " + plan.plan.tshirt + (plan.tshirt ? "（" + plan.tshirt.selected.length + " 款式）" : "")),
      ...
      function ProductCard(props) {
        ...
        var safe = Math.min(Math.max(shotIndex, 0), Math.max(shots.length - 1, 0));
        var shot = shots[safe] || null;

        function step(delta) {
          if (shots.length < 2) return;
          setShotIndex(((safe + delta) % shots.length + shots.length) % shots.length);
        }
        ...
        function open() { props.onOpen(card.groupKey, shot ? shot.id : null); }
        ...
        return h("div", { className: "ecom-card", style: { borderRadius: 10, overflow: "hidden", border: "1px solid #f0f0f2", background: "#ffffff" } },
          ...
          h("div", {
            ...
            shots.length > 1 ? h("button", { title: "上一张", onClick: function (e) { e.stopPropagation(); step(-1); }, style: arrow("left") }, "‹") : null,
            shots.length > 1 ? h("button", { title: "下一张", onClick: function (e) { e.stopPropagation(); step(1); }, style: arrow("right") }, "›") : null,
            shots.length > 1 ? h("span", {
          ...
          h("div", {
            ...
            h("div", { style: { display: "flex", alignItems: "baseline", gap: 6, marginTop: 4 } },
              ...
              h("span", { style: { ...UI.muted, fontSize: 11 } }, shot ? fmtStamp(shot.createdAt) : "")),
            ...
            card.tshirtName ? h("div", { style: { display: "flex", alignItems: "center", gap: 5, marginTop: 7, paddingTop: 6, borderTop: "1px solid #f2f2f4", minWidth: 0 } },
              h("span", { style: { flex: "0 0 auto", color: "#a9acb3", display: "flex" } }, IconShirt(13)),
              h("span", { style: { flex: 1, minWidth: 0, ...UI.muted, fontSize: 11, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, card.tshirtName)) : null));
      }
              ...
              distributeColumns(ordered.slice(0, shown), PRODUCT_COLUMNS).map(function (column, columnIndex) {
                return h("div", {
                  ...
                  return h(ProductCard, {
                    key: card.key,
                    card: card,
                    onOpen: function (groupKey, shotId) { setOpen({ groupKey: groupKey, shotId: shotId }); }
                  });
                }));
              })),
      ...
      function ProductModal(props) {
        ...
        // image rather than on the first one of the 款式.
        var focusStyle = 0;
        var focusShot = 0;
        if (props.focusId) {
          ...
          h("div", {
            ...
            h("div", { style: { flex: 1, minHeight: 0, display: "flex", gap: 12 } },
              ...
              h("div", { style: { flex: "0 0 210px", minHeight: 0, display: "flex", flexDirection: "column", gap: 10, overflowY: "auto" } },
                ...
                // The current 款式's shots as a contact sheet. The rail used to do
                // this job; now that it holds the 款式 list instead, the shots
                // need somewhere to be seen at a glance rather than one arrow
                // press at a time. 40px is enough to tell two compositions apart.
                h("div", { style: { ...UI.panel, padding: 12 } },
                  h("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 8 } },
                    h("span", { style: { color: UI.text, fontSize: 12.5, fontWeight: 600 } }, "这个款式的图集"),
                    h("span", { style: { flex: 1 } }),
                    h("span", { style: { ...UI.muted, fontSize: 11 } }, shots.length + " 张")),
                  h("div", { style: { display: "flex", flexWrap: "wrap", gap: 5 } },
                    shots.map(function (item, index) {
                      return h("button", {
                        ...
                        title: "第 " + (index + 1) + " 张",
                        style: { flex: "0 0 auto", width: 40, padding: 0, borderRadius: 7, overflow: "hidden", cursor: "pointer", background: "#f5f5f7", border: "2px solid " + (index === safeShotIndex ? "#1c1d1f" : "transparent") }
