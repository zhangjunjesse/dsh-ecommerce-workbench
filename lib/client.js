...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      /** Result tiles per screen in the 商品 grid. */
      var PRODUCT_GRID_COVER_LIMIT = 4;
      ...
      function ProductLibrary(props) {
        ...
        var openKey = openState[0]; var setOpenKey = openState[1];
        ...
        products.forEach(function (p) {
          ...
          if (!byKey[key]) { byKey[key] = { key: key, name: p.groupName || key, items: [] }; goods.push(byKey[key]); }
        ...
        if (openKey !== null && byKey[openKey]) {
          return h(ProductViewer, {
            product: byKey[openKey],
            onZoom: props.onZoom,
            onDelete: deleteProduct,
            onBack: function () { setOpenKey(null); }
          });
        }

        if (loading) {
          return h("div", { style: { flex: 1, minHeight: 0, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, ...UI.muted } }, Spinner(), "正在读成品…");
        }

        if (goods.length === 0) {
          return h("div", { style: { flex: 1, minHeight: 0, display: "flex", alignItems: "center", justifyContent: "center" } },
            h("div", { style: { maxWidth: 520, padding: "0 24px", textAlign: "center" } },
              h("div", { style: { display: "flex", justifyContent: "center", color: "#c3c6cc", marginBottom: 14 } }, IconImage(36)),
        ...
        return h("div", { style: { flex: 1, minHeight: 0, overflow: "auto", display: "flex", flexDirection: "column", padding: "2px 4px 16px" } },
          h("div", { style: { flex: "0 0 auto", display: "flex", alignItems: "center", gap: 12, margin: "2px 2px 12px" } },
            h("span", { style: { ...UI.muted } }, goods.length + " 个商品 · " + products.length + " 张成片"),
            h("span", { style: { flex: 1 } }),
            error ? h("span", { style: { color: "#b23c2e", fontSize: 12.5 } }, error) : null,
            h("button", { onClick: load, style: Object.assign({}, UI.btnGhost, { padding: "6px 11px", fontSize: 12.5 }) }, "刷新")),
          h("div", { style: { flex: "0 0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 14 } },
            goods.map(function (g) {
              var cover = g.items[0];
              var styleCount = {};
              g.items.forEach(function (p) { styleCount[p.tshirtFile] = true; });
              var stylesTotal = Object.keys(styleCount).length;
              return h("button", {
                key: g.key,
                onClick: function () { setOpenKey(g.key); },
                style: { display: "block", width: "100%", textAlign: "left", padding: 0, border: "1px solid #ececf0", borderRadius: 14, background: "#fff", overflow: "hidden", cursor: "pointer", boxShadow: "0 1px 3px rgba(20,20,25,0.04)", font: "inherit" }
              },
                // Spans, not divs: a <button> may only contain phrasing content,
                // and block children inside one are laid out unpredictably.
                h("span", { style: { position: "relative", display: "block", width: "100%", aspectRatio: "3 / 4", background: "#f5f5f7" } },
                  cover ? h("img", { src: fileUrl(cover.file), loading: "lazy", alt: "", style: { width: "100%", height: "100%", objectFit: "cover", display: "block" } }) : null,
                  h("span", { style: { position: "absolute", left: 8, top: 8, padding: "2px 8px", borderRadius: 999, background: "rgba(20,20,25,0.72)", color: "#fff", fontSize: 11.5, fontWeight: 600 } }, g.items.length + " 张")),
                h("span", { style: { display: "block", padding: "10px 12px" } },
                  h("span", { style: { display: "block", color: UI.text, fontSize: 13.5, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, g.name),
                  h("span", { style: { display: "block", ...UI.muted, marginTop: 4, fontSize: 12 } }, stylesTotal + " 个款式 · " + fmtStamp(cover ? cover.createdAt : null))));
            })));
      }
      ...
      function ProductViewer(props) {
        ...
        var styleState = React.useState(0);
