...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      /**
       * The narrowest column in the 成品库 feed. Smaller than the scene pool's:
       * this is a shelf to scan, so more, smaller tiles per screen is the point.
       */
      var PRODUCT_COL_MIN = 170;
      ...
      function ProductLibrary(props) {
        ...
        var columnCount = useColumnCount(poolRef, PRODUCT_COL_MIN);
        ...
        // it — see the note on useColumnCount: the column measurement reads the
        // element on the first mount and can never find one that appears later.
        var poolInner;
        if (loading) {
          ...
          poolInner = h("div", null,
            h("div", { style: { display: "flex", alignItems: "flex-start", gap: MASONRY_COL_GAP } },
              distributeColumns(ordered.slice(0, shown), columnCount).map(function (column, columnIndex) {
                return h("div", {
                  key: columnIndex,
        ...
        function productTile(item) {
          ...
          return h("div", { key: item.id, style: { borderRadius: 12, overflow: "hidden", border: "1px solid #ececf0", background: "#ffffff" } },
            ...
            h("div", { style: { display: "flex", alignItems: "center", gap: 6, padding: "8px 10px" } },
              h("span", { style: { flex: 1, minWidth: 0, color: UI.text, fontSize: 12.5, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, item.groupName || item.groupKey),
              h("span", { style: { flex: "0 0 auto", ...UI.muted, fontSize: 11.5 } },
                (index === undefined ? "款式" : "款式 #" + (index + 1)) + " · 第 " + (item.pass + 1) + " 轮")));
        }
        ...
        // Always rendered, whatever the state — see the note on useColumnCount.
        // The data attribute is what lets a test assert that invariant directly,
        // because "the feed silently became one column wide" is invisible to any
        // assertion about the tiles themselves.
        var feed = h("div", {
          ref: poolRef,
       ...
       * stretched to the full width of the pane. That is exactly how the 成品库
       * first shipped. Render the scroller always, with the loading and empty
       * states *inside* it.
       *
