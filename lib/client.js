...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      /**
       * A group's reference image.
       *
       * Not `/file/`: a group's screenshots live in the inbox folder on disk and are
       * never copied into the store, so `group.images` holds uploaded file names
       * rather than store ids. This route reads them where they actually are.
       */
      function groupImageUrl(groupKey, fileName) {
        return API + "/workflow/group/image?groupKey=" + encodeURIComponent(groupKey) + "&name=" + encodeURIComponent(fileName);
      }

      /**
       * A T恤's 款式, or the pre-款式 reading of a flat photo list.
       *
       * Module-level because three components need the same reading — 二创 picks a
       * photo, T恤管理 edits the buckets, the pipeline picker selects a colour —
       * and because the client has to agree with the host, which makes exactly
       * this reading in `store.normalizeTshirt`. A record created before 款式
       * existed carries `images` and nothing else, and it must resolve to the same
       * 款式 in all four places or they will disagree about what a group runs.
       */
      function colorwaysOf(ts) {
        if (Array.isArray(ts.colorways)) return ts.colorways;
        return (ts.images || []).map(function (file, index) {
          return { id: "legacy:" + file, name: "款式 " + (index + 1), white: [file], detail: [], model: [] };
        });
      }
      ...
      function injectLayoutStyle() {
        ...
        el.textContent = "." + ECOM_ROOT_CLASS + "{height:100%;min-height:0;}" +
          // 成品库的货架卡片：悬停轻微抬起，像电商列表那样"有手感"。内联样式
          // 写不了 :hover，所以这一条只能进样式表。
          ".ecom-card{transition:box-shadow .16s ease,transform .16s ease;}" +
          ".ecom-card:hover{box-shadow:0 6px 20px rgba(20,20,25,.10);transform:translateY(-1px);}" +
          // 卡片标题按淘宝那样最多两行，超出省略。
          ".ecom-clamp2{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;}";
      ...
      function Lightbox(props) {
        ...
        return h("div", {
          ...
          // Marked so an overlay underneath knows not to close itself when the
          // same Escape dismisses this one.
          "data-ecom-lightbox": "1",
          style: {
      ...
      /**
       * Ask for a short piece of text, e.g. naming a 款式.
       *
       * Returns null when there is nothing to ask with (a headless render, as in
       * the tests) — the caller treats that as "cancelled" rather than inventing a
       * name, because a silently-created 款式 is worse than one the user must name
       * themselves.
       */
      function promptAction(message, initial) {
        if (typeof window === "undefined" || typeof window.prompt !== "function") return null;
        return window.prompt(message, initial === undefined ? "" : initial);
      }
      ...
      function WorkflowSettingsCard(props) {
        ...
        return h("div", { style: { flex: "0 0 auto", background: "#ffffff", border: "1px solid #ececf0", borderRadius: 14, padding: "14px 16px", boxShadow: "0 1px 3px rgba(20,20,25,0.04)" } },
          // The name/status live in the page header now; this card is only the
          // controls, so the two do not say the same thing twice.
          h("div", { style: { display: "flex", alignItems: "center", gap: 12 } },
            h("span", { style: { flex: "1 1 auto", color: UI.text, fontSize: 13, fontWeight: 700 } }, "运行"),
            h("div", { style: { display: "flex", alignItems: "center", gap: 8, flex: "0 0 auto" } },
              ...
              h("button", {
                ...
                title: w.id === PIPELINE_WORKFLOW_ID
                  ? "处理队列里下一个「已确认」的组。要跑刚上传的那一组，请到「分组与队列」里点「估算并运行」。"
                  : "手动触发一次（停用状态下也可以）",
       ...
       *
       * Owns its own text box (so typing is not fought by the next poll), and
       * re-seeds from the host whenever the effective value changes — which is
       * how a clamped value snaps back into view instead of sitting there
       * disagreeing with what the workflow will actually use.
       */
      function SettingField(props) {
        var field = props.field;
        var value = props.value;
        var textState = React.useState(value === undefined ? "" : String(value));
        var text = textState[0]; var setText = textState[1];
        React.useEffect(function () {
        ...
        function commit() {
          var next = Math.round(Number(text));
          if (!isFinite(next)) { setText(String(value)); return; }
          if (next === Number(value)) return;
          props.onChange(next);
        }

        var inputStyle = { width: 88, border: "1px solid #e5e5ea", outline: 0, borderRadius: 9, padding: "6px 9px", fontSize: 12.5, color: UI.text, background: "#fff", boxSizing: "border-box" };

        if (field.type === "checkbox") {
          return h("label", { style: { flex: "0 0 auto", display: "flex", alignItems: "center", gap: 8, padding: "4px 0", cursor: "pointer" } },
            h("input", { type: "checkbox", checked: value === true, onChange: function (e) { props.onChange(e.target.checked); } }),
            h("span", { style: { fontSize: 12.5, color: UI.text } }, field.label));
        }

        return h("div", { style: { flex: "1 1 260px", display: "flex", alignItems: "center", gap: 10, padding: "3px 0" } },
          h("span", { style: { flex: "1 1 auto", minWidth: 0, fontSize: 12.5, color: UI.text } }, field.label),
          h("input", {
            value: text,
            inputMode: "numeric",
            ...
            title: (typeof field.min === "number" && typeof field.max === "number") ? ("范围 " + field.min + " – " + field.max) : "",
            style: inputStyle
          }),
          h("span", { style: { flex: "0 0 auto", ...UI.muted, fontSize: 11.5 } },
            (typeof field.min === "number" && typeof field.max === "number") ? (field.min + "–" + field.max) : ""));
      }

      /**
       * The workflow's declared settings, as a form.
       *
       * Generated from `settingFields`, which comes from the definition in code —
       * so adding a knob to a workflow adds it to this form with no client change.
       */
      function WorkflowSettingsForm(props) {
        var w = props.workflow;
        var fields = w.settingFields || [];
        if (fields.length === 0) return null;
        var values = w.settings || {};
        return h("div", { style: { flex: "0 0 auto", marginTop: 12, padding: "12px 14px", borderRadius: 14, background: "#ffffff", border: "1px solid #ececf0" } },
          h("div", { style: { display: "flex", alignItems: "center", gap: 10, marginBottom: 4 } },
            h("span", { style: { color: UI.text, fontSize: 13, fontWeight: 700 } }, "参数"),
            h("span", { style: { ...UI.muted } }, "改动立即保存，下一次运行生效")),
          h("div", { style: { display: "flex", flexWrap: "wrap", gap: "0 28px" } },
            fields.map(function (field) {
              return h("div", { key: field.key, style: { flex: "1 1 280px", minWidth: 0, paddingBottom: 4 } },
                h(SettingField, {
                  field: field,
                ...
                field.help ? h("div", { style: { color: "#a9acb3", fontSize: 11.5, lineHeight: 1.6, margin: "-2px 0 4px" } }, field.help) : null);
            })));
      }
      ...
      function WorkflowDetail(props) {
        ...
        // Forwarded to PipelinePanel, which renders the 印花流水线 group rows; the
        // T恤 picker on those rows needs the list, and it is already loaded.
        ...
        function loadRuns() {
          return apiGet("/workflow/runs?workflowId=" + encodeURIComponent(w.id) + "&limit=50").then(function (res) {
            ...
            var list = res.runs || [];
            setRuns(list);
            // Landing on the history tab should show a log, not an instruction
            // to go and pick one: the newest run is what anyone came here for.
            if (detailRef.current === null && list.length > 0) loadDetail(list[0].id);
          }, function () {});
        }
        ...
        var history = h("div", { style: { flex: "0 0 auto" } },
          ...
          runs.length === 0
            ...
            : h("div", { style: { display: "flex", gap: 12, height: "100%", minHeight: 260 } },
        ...
        // A workflow's page is a place to *do* one thing at a time: stacks of
        // equally-weighted panels are what made this page unreadable. The queue
        // leads for a workflow that has one, because that is the daily surface;
        // settings and history are one click away and out of the way.
        var tabs = [];
        if (w.id === PIPELINE_WORKFLOW_ID) tabs.push({ key: "groups", label: "分组与队列" });
        tabs.push({ key: "settings", label: "设置与参数" });
        tabs.push({ key: "runs", label: "运行记录" });
        var tabState = React.useState(tabs[0].key);
        var tab = tabState[0]; var setTab = tabState[1];

        var status = workflowStatus(w.lastStatus, w.running);

        return h("div", { style: { flex: 1, minHeight: 0, display: "flex", flexDirection: "column", padding: "2px 4px 12px" } },
          h("div", { style: { flex: "0 0 auto", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 6 } },
            h("button", {
              onClick: function () { props.onBack(); },
              style: { display: "inline-flex", alignItems: "center", gap: 6, padding: "5px 10px", borderRadius: 9, border: "1px solid #e5e5ea", background: "#fff", color: UI.text2, cursor: "pointer", fontSize: 12.5 }
            }, "← 工作流"),
            h("span", { style: { color: UI.text, fontSize: 15, fontWeight: 700 } }, w.name || w.id),
            h("span", { style: { flex: "0 0 auto", padding: "2px 8px", borderRadius: 999, fontSize: 11.5, fontWeight: 600, background: status.bg, color: status.color } }, status.text),
            w.running ? h("span", { style: { display: "flex", alignItems: "center", gap: 6, color: "#1f5fa9", fontSize: 12 } }, Spinner(), "运行中") : null,
            h("span", { style: { flex: 1 } }),
            h("span", { style: { ...UI.muted } }, "上次运行 " + fmtStamp(w.lastRunAt))),
          h("div", { style: { flex: "0 0 auto", ...UI.muted, marginBottom: 12 } }, w.description || ""),

          h("div", { style: { flex: "0 0 auto", display: "flex", gap: 4, marginBottom: 12, borderBottom: "1px solid #ececf0" } },
            tabs.map(function (entry) {
              var active = entry.key === tab;
              return h("button", {
                key: entry.key,
                onClick: function () { setTab(entry.key); },
                style: {
                  border: 0, background: "transparent", cursor: "pointer", font: "inherit",
                  padding: "7px 12px", marginBottom: -1, fontSize: 13,
                  fontWeight: active ? 600 : 400, color: active ? UI.text : UI.text2,
                  borderBottom: "2px solid " + (active ? "#1c1d1f" : "transparent")
                }
              }, entry.label);
            })),
          ...
          h("div", { style: { flex: 1, minHeight: 0, overflow: "auto", paddingRight: 2 } },
            tab === "groups" && w.id === PIPELINE_WORKFLOW_ID ? h(PipelinePanel, {
            ...
            tab === "settings" ? h("div", { style: { display: "flex", flexDirection: "column", gap: 12 } },
              ...
              h(WorkflowSettingsForm, {
                workflow: w,
                onSave: function (patch) { saveConfig({ settings: patch }); }
              })) : null,
            tab === "runs" ? history : null));
      }
      ...
      function PipelinePanel(props) {
        ...
        // The T恤 the group rows can pick from. Absent means "no picker data",
        // which the row renders as 还没有 T恤 rather than as an empty selection.
        var tshirts = props.tshirts || [];
        // Whether the workbench is showing 工作流 at all — see the paste listener.
        ...
        // Each group's T恤 choice is a draft keyed by groupKey; the group record's
        // own tshirtId/tshirtImages is what the host has stored. Keeping them apart
        // is deliberate: merely picking a T恤 must not write, or clicking one would
        // re-queue a group that already finished. The draft is handed over by
        // approve (「排队」/「只确认」) and by run (「确认并立即运行」).
        var tshirtDraftState = React.useState({});
        var tshirtDraft = tshirtDraftState[0]; var setTshirtDraft = tshirtDraftState[1];
        var tshirtPickerState = React.useState(null);
        var tshirtPickerFor = tshirtPickerState[0]; var setTshirtPickerFor = tshirtPickerState[1];
        // The T恤 picked next to 分组名 while creating one. It belongs to no group
        // yet — the group does not exist — so it is held separately and stored
        // against the new group as soon as 创建分组 comes back with a key.
        var newGroupTshirtState = React.useState(null);
        var newGroupTshirt = newGroupTshirtState[0]; var setNewGroupTshirt = newGroupTshirtState[1];
        var newGroupPickerState = React.useState(false);
        var newGroupPickerOpen = newGroupPickerState[0]; var setNewGroupPickerOpen = newGroupPickerState[1];
        ...
        /**
         * A T恤 choice, shaped for the approve route.
         *
         * Empty when nothing was picked, so a caller that never touched the picker
         * sends exactly what it used to send — the host keeps its stored value (or
         * its "first T恤, all photos" default) and this stays invisible.
         */
        function tshirtPayloadOf(choice) {
          if (!choice) return {};
          var payload = {};
          if (typeof choice.tshirtId === "string" && choice.tshirtId !== "") payload.tshirtId = choice.tshirtId;
          if (Array.isArray(choice.tshirtColorways)) payload.tshirtColorways = choice.tshirtColorways;
          return payload;
        }

        /**
         * Store a T恤 choice for one group *without* changing whether it is queued.
         *
         * `approved` mirrors the group's current queue state rather than being
         * forced true: setting a T恤 must not re-queue a group that already
         * finished, and `false` on an unqueued group is a no-op on the host. A
         * group that does not exist yet reads as unqueued, which is also correct.
         */
        function saveTshirtChoice(groupKey, choice) {
          if (groupKey === null || groupKey === undefined) return Promise.resolve(null);
          var payload = tshirtPayloadOf(choice);
          if (Object.keys(payload).length === 0) return Promise.resolve(null);
          var current = groups.filter(function (g) { return g.key === groupKey; })[0] || null;
          var queued = current !== null && current.approvedAt !== null;
          return apiPost("/workflow/group/approve", Object.assign({ groupKey: groupKey, approved: queued }, payload));
        }
        function runGroup(groupKey, force) {
          setError("");
          setBusy("run");
          // Running is the other way a group's T恤 selection has to reach the
          // host: only `/workflow/group/approve` stores it, and 「确认并立即运行」
          // does not go through that route. Without this the picker would look
          // like it worked and the run would keep using whatever was stored last.
          //
          // `saveTshirtChoice` mirrors the group's current queue state rather than
          // forcing true: setting a T恤 must not re-queue a group that already
          // finished, and `false` on an unqueued group is a no-op on the host.
          saveTshirtChoice(groupKey, tshirtDraft[groupKey]).then(function () {
            return apiPost("/workflow/run", { id: PIPELINE_WORKFLOW_ID, params: { groupKey: groupKey, force: force === true } });
          }).then(function (res) {
            setBusy("");
        ...
        function approveGroup(groupKey, approved) {
          setError("");
          // Approving is where the host stores a group's T恤 selection, so the
          // draft rides along with it — that is why the picker has no save button.
          var payload = Object.assign({ groupKey: groupKey, approved: approved }, tshirtPayloadOf(tshirtDraft[groupKey]));
          apiPost("/workflow/group/approve", payload).then(function (res) {
        ...
        /**
         * Paste straight into the group being composed.
         *
         * Same staging as 选择图片 — nothing is written until 创建分组 — so the two
         * ways in converge on one place, and a pasted screenshot is just another
         * pending tile that can be removed before submitting.
         *
         * Document-level and gated on `active` for the same two reasons 场景图管理's
         * listener is: every view stays mounted (hidden with `display:none`), so
         * without the gate an image pasted into 印花提取 would land here; and the
         * caret's own field wins, because the chat composer below the workbench is
         * one and an image pasted there must stay the chat's business.
         */
        React.useEffect(function () {
        ...
        function createGroup() {
          ...
          setError("");
          setBusy("upload");
          // Captured before the request: the create form's picker may be closed by
          // the time this resolves, and the value on screen when 创建分组 was
          // pressed is the one the user meant.
          var chosen = newGroupTshirt;
          apiPost("/workflow/group/upload", {
            ...
            var key = res.group && res.group.key ? res.group.key : null;
            // The T恤 picked in this form belongs to the group that was just made,
            // so it is stored *before* the estimate opens — otherwise the price
            // would be worked out against the old choice and read as a lie.
            saveTshirtChoice(key, chosen).then(function () {
              setNewGroupTshirt(null);
        ...
        var uploadBar = h("div", { style: { flex: "0 0 auto", marginBottom: 12, padding: "10px 12px", borderRadius: 12, background: "#f7f8fa", border: "1px solid #ececf0" } },
          h("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" } },
            ...
            h("span", { style: { ...UI.muted, flex: "0 0 auto" } }, pending.length > 0 ? "已选 " + pending.length + " 张" : "还没有选图，也可以直接粘贴"),
            h("span", { style: { flex: "1 1 auto" } }),
            h("button", {
              onClick: createGroup,
          ...
          // The T恤 this group will run against, chosen *here* rather than only on
          // the row afterwards: creating a group is the moment you are setting the
          // thing up, and a control that appears on a row you have not made yet is
          // not somewhere anyone looks. The row keeps its own picker for changing
          // the choice later.
          h("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginTop: 9, paddingTop: 9, borderTop: "1px dashed #e3e5ea" } },
            h("span", { style: { ...UI.muted, fontSize: 11.5, flex: "0 0 auto" } }, "T恤"),
            h("button", {
              onClick: function () { setNewGroupPickerOpen(!newGroupPickerOpen); },
              title: "新建的这一组跑哪件 T恤、哪几个款式",
              style: Object.assign({}, UI.btnGhost, { padding: "5px 11px", fontSize: 12 }, newGroupPickerOpen ? { borderColor: "#cfdff7", background: "#f2f7ff", color: "#1c4d9c" } : null)
            }, tshirtChoiceLabel(newGroupChoice()) + " ▾")),
          newGroupPickerOpen ? tshirtPickerBlock(
            newGroupChoice(),
            function (tshirtId, tshirtColorways) { setNewGroupTshirt({ tshirtId: tshirtId, tshirtColorways: tshirtColorways }); },
            function () { setNewGroupPickerOpen(false); }
          ) : null,
        ...
        /**
         * Which T恤 a group will actually run against.
         *
         * Mirrors the host's `resolveTshirt`: an explicit choice wins, otherwise it
         * is the first T恤 and **all** of its photos. The row shows this resolved
         * value instead of "未选", because the default is a real choice the run is
         * about to make — the selection existed on the host from the start and was
         * only ever invisible in the UI, which is exactly why it looked broken.
         *
         * A stored choice can be stale (the photo was deleted, or the T恤 was
         * switched), so the photos are filtered against the T恤 that survived;
         * filtering down to nothing means "all 款式", the same meaning an empty
         * list has on the host.
         */
        /**
         * A group's effective T恤 choice, resolved the way the host resolves it.
         *
         * The unit is the **款式** (one colour): picking 白色T恤 means its 白底图 —
         * front and back — and, downstream, its 细节图.
         *
         * A stored choice can be stale (the 款式 was deleted, or the T恤 was
         * switched), so it is filtered against the 款式 that survived; filtering to
         * nothing means "all 款式", the same meaning an empty list has on the host.
         * `tshirtImages` — a per-photo selection from before 款式 existed — is read
         * as "the 款式 that own those photos".
         */
        function resolveTshirtChoice(tshirtId, colorwayIds, imageFiles) {
          var id = tshirtId || null;
          var ts = (id === null ? null : tshirts.filter(function (t) { return t.id === id; })[0]) || tshirts[0] || null;
          if (ts === null) return null;
          var all = colorwaysOf(ts);
          var pickedIds = Array.isArray(colorwayIds) ? colorwayIds : [];
          var pickedFiles = Array.isArray(imageFiles) ? imageFiles : [];
          var picked;
          if (pickedIds.length > 0) {
            picked = all.filter(function (c) { return pickedIds.indexOf(c.id) !== -1; });
          } else if (pickedFiles.length > 0) {
            picked = all.filter(function (c) {
              return c.white.some(function (f) { return pickedFiles.indexOf(f) !== -1; });
            });
          } else {
            picked = [];
          }
          return { ts: ts, colorways: all, picked: picked, custom: picked.length > 0 };
        }
        function groupTshirtChoice(group) {
          var draft = tshirtDraft[group.key];
          if (draft) {
            return resolveTshirtChoice(draft.tshirtId, draft.tshirtColorways, null);
          }
          return resolveTshirtChoice(
            group.tshirtId || null,
            Array.isArray(group.tshirtColorways) ? group.tshirtColorways : [],
            Array.isArray(group.tshirtImages) ? group.tshirtImages : []
          );
        }

        /** The choice 创建分组 is set to use — same resolution, no group yet. */
        function newGroupChoice() {
          return resolveTshirtChoice(
            newGroupTshirt ? newGroupTshirt.tshirtId : null,
            newGroupTshirt ? newGroupTshirt.tshirtColorways : null,
            null
          );
        }

        /** One-line summary of a group's choice; only the row renders this. */
        function tshirtChoiceLabel(choice) {
          if (choice === null) return "还没有 T恤";
          if (!choice.custom) return choice.ts.name + " · 全部 " + choice.colorways.length + " 个款式";
          if (choice.picked.length === 1) return choice.ts.name + " · " + choice.picked[0].name;
          return choice.ts.name + " · 选了 " + choice.picked.length + " 个款式";
        }

        /**
         * The T恤 picker body, shared by 创建分组 and the group row.
         *
         * Two levels: pick the T恤, then tick the 款式 (colours) it runs against.
         * Ticking nothing means every 款式, which is what the host does with an
         * empty list — so the default is a state the user can get back to rather
         * than a one-way switch.
         *
         * The 细节图 are shown per 款式 but ticked with it, never separately: a
         * collar close-up belongs to the colour it was shot on, and letting the two
         * be chosen apart is exactly the mismatch this grouping exists to prevent.
         *
         * `pick` receives `{tshirtId, tshirtColorways}`; the two callers store it
         * differently — the create form against the group it is about to make, the
         * row against that group's key. There is no save button on purpose: the
         * value is handed to the host by 「创建分组」 and by 「排队」/「只确认」/
         * 「确认并立即运行」, the only ways a group starts.
         */
        function tshirtPickerBlock(choice, pick, onClose) {
          var panel = { padding: "10px 12px 12px", borderTop: "1px solid #e6e9ef", background: "#f8fafc", borderRadius: "0 0 12px 12px" };
          if (tshirts.length === 0) {
            return h("div", { style: panel },
              h("div", { style: { color: UI.text, fontSize: 12.5, fontWeight: 700, marginBottom: 6 } }, "这一组用哪件 T恤"),
              h("div", { style: { ...UI.muted, fontSize: 12 } }, "还没有 T恤。先到「T恤管理」加一件，再回来选。"));
          }
          var tiles = tshirts.map(function (t) {
            var on = choice !== null && choice.ts.id === t.id;
            var first = (t.images || [])[0];
            return h("button", {
              key: t.id,
              onClick: function () { pick(t.id, []); },
              title: t.name + "（" + colorwaysOf(t).length + " 个款式）",
              style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 5, width: 78, padding: "7px 4px", borderRadius: 10, border: on ? "2px solid #1c1d1f" : "1px solid #ececf0", background: "#fff", cursor: "pointer", font: "inherit" }
            },
              Thumb({ src: first ? fileUrl(first) : null, size: 48, label: t.name }),
              h("span", { style: { fontSize: 11, color: on ? UI.text : UI.text2, maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, t.name));
          });
          var pickedIds = choice === null ? [] : choice.picked.map(function (c) { return c.id; });
          var colorwayTiles = choice === null ? [] : choice.colorways.map(function (cw) {
            var on = pickedIds.indexOf(cw.id) !== -1;
            var first = (cw.white || [])[0];
            var counts = "白底图 " + cw.white.length
              + (cw.detail.length > 0 ? " · 细节图 " + cw.detail.length : "")
              + ((cw.model || []).length > 0 ? " · 模特 " + cw.model.length : "");
            return h("button", {
              key: cw.id,
              onClick: function () {
                pick(choice.ts.id, on ? pickedIds.filter(function (x) { return x !== cw.id; }) : pickedIds.concat([cw.id]));
              },
              title: cw.name + "（" + counts + "）",
              style: { position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, width: 92, padding: "7px 4px", borderRadius: 10, border: on ? "2px solid #1c1d1f" : "1px solid #ececf0", background: "#fff", cursor: "pointer", font: "inherit" }
            },
              Thumb({ src: first ? fileUrl(first) : null, size: 52, label: cw.name, onZoom: onZoom }),
              h("span", { style: { fontSize: 11, fontWeight: on ? 700 : 400, color: on ? UI.text : UI.text2, maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, cw.name),
              h("span", { style: { fontSize: 10, color: UI.muted.color, maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, counts),
          ...
          return h("div", { style: panel },
            h("div", { style: { display: "flex", alignItems: "center", gap: 10, marginBottom: 8 } },
              h("span", { style: { color: UI.text, fontSize: 12.5, fontWeight: 700 } }, "这一组用哪件 T恤"),
              h("span", { style: { flex: 1 } }),
              h("button", { onClick: onClose, style: Object.assign({}, UI.btnGhost, { padding: "4px 9px", fontSize: 12 }) }, "收起")),
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 10 } }, tiles),
            h("div", { style: { ...UI.muted, fontSize: 11.5, marginBottom: 6 } },
              choice === null
                ? "选一件 T恤。"
                : "不勾就是这件 T恤的全部 " + choice.colorways.length + " 个款式；勾了哪个颜色，就只用那个颜色的白底图和它的细节图。"),
            h("div", { style: { display: "flex", flexWrap: "wrap", gap: 8 } }, colorwayTiles));
        }

        /**
         * The cost preview for one group, rendered *inside that group's row*.
         *
         * It used to be a box appended under the whole list, so clicking a row
         * near the top put the answer off-screen and the three buttons that
         * belong to that decision nowhere near the row they act on.
         */
        function estimateBlock(plan) {
          return h("div", { style: { padding: "10px 12px 12px", borderTop: "1px solid #f0e2b8", background: "#fffdf5", borderRadius: "0 0 12px 12px" } },
          h("div", { style: { display: "flex", alignItems: "center", gap: 10, marginBottom: 8 } },
            h("span", { style: { color: UI.text, fontSize: 12.5, fontWeight: 700 } }, "这一组要花多少"),
            h("span", { style: { flex: 1 } }),
            h("button", { onClick: function () { setEstimate(null); }, style: Object.assign({}, UI.btnGhost, { padding: "4px 9px", fontSize: 12 }) }, "收起")),
          h("div", { style: { display: "flex", gap: 14, flexWrap: "wrap", fontSize: 12.5, color: UI.text2, marginBottom: 8 } },
            h("span", { key: "b" }, "提取印花 " + plan.plan.extract),
            h("span", { key: "c" }, "印花二创 " + plan.plan.recreate + "（" + plan.prompts.recreate.length + " 个提示词 × " + plan.settings.recreateOutputs + "）"),
            h("span", { key: "d" }, "T恤融合 " + plan.plan.tshirt + (plan.tshirt
              // 款式 is the colour, and one 融合 is made per colour — so this shows
              // both numbers. The photo count alone read as "6 款式" for a 3-colour
              // T恤 shot twice each, which is what it was showing.
              ? "（" + plan.tshirt.colorways.length + " 个款式 · " + plan.tshirt.selected.length + " 张白底图）"
              : "")),
            h("span", { key: "e" }, "换装裂变 " + plan.plan.scene + "（每件 " + plan.settings.sceneCount + " 张场景图 × 每张 " + plan.settings.sceneOutputs + " 张）")),
          h("div", { style: { fontSize: 13, color: UI.text, fontWeight: 600, marginBottom: 8 } },
            "合计 " + plan.plan.total + " 次生成调用，本次还需 " + plan.pending.total + " 次" +
            (plan.pending.total < plan.plan.total ? "（其余已完成，会自动跳过）" : "") +
            "；单次上限 " + plan.cap.max + " 次"),
          plan.warnings.length > 0 ? h("div", { style: { marginBottom: 8 } },
            plan.warnings.map(function (warning, index) {
              return h("div", { key: index, style: { color: "#8a6d1f", fontSize: 12, lineHeight: 1.7 } }, "· " + warning);
            })) : null,
          plan.cap.exceeded ? h("div", { style: { color: "#b23c2e", fontSize: 12.5, marginBottom: 8 } },
            "已超过单次上限，直接运行会被拒绝。请减少款式或参考图，或调高 ECOM_WORKFLOW_MAX_CALLS。") : null,
          h("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" } },
            h("button", {
              disabled: busy === "run",
              onClick: function () { runGroup(plan.group.key, false); },
              style: Object.assign({}, UI.btnPrimary, busy === "run" ? { opacity: 0.6 } : null)
            }, busy === "run" ? "正在启动…" : "确认并立即运行"),
            h("button", {
              onClick: function () { approveGroup(plan.group.key, true); setEstimate(null); },
              style: UI.btnGhost
            }, "只确认，交给周期跑"),
            h("button", {
              disabled: busy === "run",
              onClick: function () { runGroup(plan.group.key, true); },
        ...
        var groupList = groups.length === 0
          ...
          : h("div", { style: { flex: "0 0 auto", display: "flex", flexDirection: "column", gap: 8 } },
              groups.map(function (group) {
                ...
                var queued = group.approvedAt !== null;
                var isOpen = results !== null && results.key === group.key;
                // 四步的计数连成一行；缺计数就说还没产出，而不是留一块空白。
                var counts = group.counts
                  ? "印花 " + group.counts.extract + " · 二创 " + group.counts.recreate + " · T恤 " + group.counts.tshirt + " · 成片 " + group.counts.scene
                  : "还没有产出";
                var small = function (extra) { return Object.assign({}, UI.btnGhost, { padding: "4px 9px", fontSize: 12 }, extra || null); };
                // 两行：第一行回答「这是哪一组、什么状态、下一步做什么」，第二行
                // 才是计数与次要动作。之前六个控件挤在一行，谁主谁次看不出来。
                return h("div", { key: group.key, style: { borderRadius: 12, background: "#fff", border: "1px solid " + (group.status === "running" ? "#cfdff7" : "#ececf0") } },
                  h("div", { style: { display: "flex", alignItems: "center", gap: 9, padding: "9px 12px 0" } },
                    h("span", { style: { flex: "0 0 auto", width: 8, height: 8, borderRadius: "50%", background: chip.color } }),
                    h("span", { title: (group.images || []).join("、"), style: { color: UI.text, fontSize: 13, fontWeight: 600, flex: "0 1 auto", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, group.name),
                    ...
                    h("span", { style: { ...UI.muted, fontSize: 12, flex: "0 0 auto" } }, group.imageCount + " 张参考图"),
                    h("span", { style: { flex: 1, minWidth: 8 } }),
                    h("button", { onClick: function () { openEstimate(group.key); }, disabled: busy === "estimate", style: Object.assign({}, UI.btnPrimary, { padding: "5px 12px", fontSize: 12.5 }, busy === "estimate" ? { opacity: 0.6 } : null) }, "估算并运行"),
                    h("button", { onClick: function () { deleteGroup(group); }, title: "从收图目录移除（已生成的成品保留）", style: { flex: "0 0 auto", width: 26, height: 26, borderRadius: 8, border: "1px solid #ececf0", background: "#fff", color: UI.text2, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", justifyContent: "center" } }, IconTrash(13))),
                  h("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "7px 12px 9px" } },
                    h("span", { style: { ...UI.muted, fontSize: 11.5, flex: "1 1 auto", minWidth: 0 } }, counts),
                    h("button", {
                      onClick: function () { approveGroup(group.key, !queued); },
                      title: queued ? "已交给周期调度，点击取消排队" : "交给周期调度，一组一组地跑",
                      style: small(queued ? { borderColor: "#cfe3d7", background: "#f2fbf6", color: "#1c7a4a" } : null)
                    }, queued ? "✓ 已排队" : "排队"),
                    isOpen
                      ? h("button", { onClick: function () { setResults(null); }, style: small() }, "收起成品")
                      : h("button", { onClick: function () { loadOutputs(group.key); }, style: small() }, "看成品")),
                  // 这一组的参考图。分组的名字就是「组3」，光看"3 张参考图"认不出是哪一组
                  // —— 三个不同商品的截图在缩略图出来之前长得一模一样。
                  (group.images && group.images.length > 0) ? h("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "0 12px 9px", minWidth: 0 } },
                    h("span", { style: { ...UI.muted, fontSize: 11.5, flex: "0 0 auto" } }, "参考图"),
                    h("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", minWidth: 0 } },
                      group.images.map(function (file) {
                        return h("div", { key: file, style: { flex: "0 0 auto" } },
                          Thumb({ src: groupImageUrl(group.key, file), size: 40, label: group.name + " · " + file, onZoom: onZoom }));
                      }))) : null,
                  // 这一组跑哪件 T恤。它单独占一行而不是挤进上一行：上一行是
                  // 「这组现在什么状态」，这一行是「这组要拿什么去跑」，两件事。
                  h("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "0 12px 9px" } },
                    h("span", { style: { ...UI.muted, fontSize: 11.5, flex: "0 0 auto" } }, "T恤"),
                    h("button", {
                      onClick: function () { setTshirtPickerFor(tshirtPickerFor === group.key ? null : group.key); },
                      title: "这一组跑哪件 T恤、哪几个款式",
                      style: small(tshirtPickerFor === group.key ? { borderColor: "#cfdff7", background: "#f2f7ff", color: "#1c4d9c" } : null)
                    }, tshirtChoiceLabel(groupTshirtChoice(group)) + " ▾"),
                    h("span", { style: { flex: 1, minWidth: 8 } })),
                  tshirtPickerFor === group.key ? tshirtPickerBlock(groupTshirtChoice(group), function (tshirtId, tshirtColorways) {
                    var choice = { tshirtId: tshirtId, tshirtColorways: tshirtColorways };
                    setTshirtDraft(function (prev) {
                      var next = Object.assign({}, prev);
                      next[group.key] = choice;
                      return next;
                    });
                    // Written through at once, not held until 排队. The estimate is
                    // priced from what the host has stored, so a draft that only
                    // landed at approve time left the row saying one thing and the
                    // price quoting another — you could tick one 款式 and watch the
                    // 225 not move. `saveTshirtChoice` mirrors the group's queue
                    // state, so writing here cannot re-queue a finished group.
                    saveTshirtChoice(group.key, choice).then(function () {
                      if (estimate !== null && estimate.group && estimate.group.key === group.key) openEstimate(group.key);
                    });
                  }, function () { setTshirtPickerFor(null); }) : null,
                  // 展开区贴着那一行，而不是跑到列表末尾。
                  estimate !== null && estimate.group.key === group.key ? estimateBlock(estimate) : null,
                  isOpen ? resultsBlock() : null);
              }));

        /** One group's产出, rendered inside that group's row for the same reason. */
        function resultsBlock() {
          if (results === null) return null;
          var assets = groupAssets(results.key);
          return h("div", { style: { padding: "11px 12px 12px", borderTop: "1px solid #ececf0", background: "#fcfcfd", borderRadius: "0 0 12px 12px" } },
            h("div", { style: { display: "flex", alignItems: "center", gap: 10, marginBottom: 4 } },
              h("span", { style: { color: UI.text, fontSize: 12.5, fontWeight: 700 } }, "这一组的产出"),
              h("span", { style: { ...UI.muted, fontSize: 12 } },
                "共 " + (assets.prints.length + assets.recreated.length + assets.composites.length + results.total) + " 张"),
              h("span", { style: { flex: 1 } }),
              h("button", { onClick: function () { loadOutputs(results.key); if (refresh) refresh(); }, style: Object.assign({}, UI.btnGhost, { padding: "4px 9px", fontSize: 12 }) }, "刷新")),
            h("div", { style: { ...UI.muted, fontSize: 11.5, marginBottom: 6 } },
      ...
      /**
       * Which scene a shot was taken in.
       *
       * `sceneIndex` on records written since the settings work; `pass` on the
       * ones written before it, when the same number meant the same thing. Both
       * are read so an existing shelf keeps its「N 个场景」line instead of
       * collapsing to one.
       */
      function sceneOf(item) {
        if (!item) return 0;
        if (item.sceneIndex !== undefined && item.sceneIndex !== null) return item.sceneIndex;
        return item.pass === undefined || item.pass === null ? 0 : item.pass;
      }

      /**
       * A shot's ratio as a listing tag (「3:4」), or null.
       *
       * Only for ratios that reduce to something a person would write down: a
       * 1913×864 render would reduce to "1913:864", which is noise on a card, so
       * an odd measurement simply gets no tag rather than an unreadable one.
       */
      function ratioLabel(item) {
        var width = Math.round(Number(item && item.width));
        var height = Math.round(Number(item && item.height));
        if (!isFinite(width) || !isFinite(height) || width <= 0 || height <= 0) return null;
        function gcd(a, b) { return b === 0 ? a : gcd(b, a % b); }
        var divisor = gcd(width, height) || 1;
        var w = width / divisor;
      ...
      /**
       * One 款式 (SKU) in the 成品库 feed, shaped like a **Taobao listing card**:
       * a square image stage on a light ground, then the title / spec / tag /
       * shop lines underneath.
       *
       * Why this shape: the shelf exists to judge goods *as listings*, and the
       * listing is where they are headed. Taobao's own card is the reference —
       * square image, bold two-line title, one strong spec line where the price
       * sits, small tags, and the shop underneath.
       *
       * What is deliberately NOT copied: price, 销量 and 店铺评分. We have no such
       * data, and inventing numbers on a shelf used for deciding what to publish
       * would be worse than leaving the slot out. Every line here is something
       * the pipeline actually recorded — how many shots this 款式 has, how many
       * scenes they came from, when they were made, and which T恤 it is a
       * colourway of (the closest thing we have to a shop).
       *
       * One image per 款式, with the rest behind a swipe or the arrows: a 商品
       * with 24 款式 would otherwise be 192 near-identical tiles and the eye
       * would have nothing to compare.
       *
       * The stage is a fixed square with the shot **contained**. That is Taobao's
       * tile without Taobao's crop — these are 3:4 shots and cropping one to a
       * square cuts the garment. A square also means the grid can never jump as
       * images load, which is the problem the measured-ratio version was working
       * around.
       */
      function ProductCard(props) {
        var card = props.card;
        var shots = card.items;
        var shotState = React.useState(0);
        var shotIndex = shotState[0]; var setShotIndex = shotState[1];
        var dragRef = React.useRef(null);
        var draggedRef = React.useRef(false);

        // The cover is the **白底图** — the 融合 output this whole card is named after —
        // and the 成片 follow it, exactly as they do in the product page's strip.
        //
        // A 成片 is the wrong cover for a shelf: the shots of one 款式 are ONE garment in
        // eight different scenes, so a grid of them reads as eight products rather than
        // as one, and the backgrounds (street, indoors, day, night) make the grid noisy.
        // Against white the only thing that changes from card to card is the print,
        // which is the thing being chosen. It is also what a real listing leads with.
        var slides = card.styleFile
          ? [{ key: "main:" + card.styleFile, file: card.styleFile }].concat(shots)
          : shots;

        var safe = Math.min(Math.max(shotIndex, 0), Math.max(slides.length - 1, 0));
        var slide = slides[safe] || null;

        function step(delta) {
          if (slides.length < 2) return;
          setShotIndex(((safe + delta) % slides.length + slides.length) % slides.length);
        }

        // `card.key` is the composite "group|style" key the list is keyed by —
        // NOT the 商品 key. Passing it as the group was a real bug: the page
        // lookup then missed and clicking a card did nothing at all.
        //
        // On the cover there is no shot to name, and `null` opens the product page on
        // its 产品展示图 — the same image the card was already showing.
        function open() { props.onOpen(card.groupKey, slide && slide.id ? slide.id : null, card.styleFile); }

        // 这个款式拍过几个场景 —— 第四步是"每件 N 张场景图 × 每张 M 张"，所以
        // 张数 ÷ 场景数就是每个场景出几张，两行都是真实记录里的数。
        var sceneSeen = {};
        shots.forEach(function (item) { sceneSeen[sceneOf(item)] = true; });
        var sceneCount = Object.keys(sceneSeen).length;
        var tags = [];
        if (sceneCount > 0) tags.push(sceneCount + " 个场景");
        if (sceneCount > 0) tags.push("每场景 " + Math.round(shots.length / sceneCount) + " 张");
        if (card.ratio) tags.push(card.ratio);

        var arrow = function (side) {
          return {
            position: "absolute", top: "50%", transform: "translateY(-50%)",
            [side]: 4, width: 24, height: 24, padding: 0, borderRadius: "50%",
            border: "1px solid rgba(255,255,255,.55)", background: "rgba(20,20,22,.45)",
            color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 14, lineHeight: 1
          };
        };

        var title = card.groupName + " · 款式 #" + (card.styleIndex + 1);

        return h("div", { className: "ecom-card", style: { borderRadius: 10, overflow: "hidden", border: "1px solid #f0f0f2", background: "#ffffff" } },
          // 方图舞台：淘宝的方格子，但图是 contain 的，不裁。
          h("div", {
            style: { position: "relative", background: "#f4f4f6", aspectRatio: "1 / 1", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", touchAction: "pan-y" },
            onPointerDown: function (e) { dragRef.current = { x: e.clientX }; draggedRef.current = false; },
            ...
            onPointerLeave: function () { dragRef.current = null; }
          },
            slide ? h("img", {
              src: fileUrl(slide.file),
              alt: "",
              loading: "lazy",
              draggable: false,
              onClick: function () {
                // A drag that happened to end on the image must not also open the
                // page — otherwise swiping through shots fires the navigation.
                if (draggedRef.current) { draggedRef.current = false; return; }
                open();
              },
              // 由舞台界定尺寸（maxWidth/maxHeight），不按文件尺寸撑开：舞台是
              // 定比例的方框，图再长也不会把卡片顶变形。
              style: { maxWidth: "100%", maxHeight: "100%", width: "auto", height: "auto", objectFit: "contain", cursor: "pointer", display: "block" }
            }) : null,
            slides.length > 1 ? h("button", { title: "上一张", onClick: function (e) { e.stopPropagation(); step(-1); }, style: arrow("left") }, "‹") : null,
            slides.length > 1 ? h("button", { title: "下一张", onClick: function (e) { e.stopPropagation(); step(1); }, style: arrow("right") }, "›") : null,
            slides.length > 1 ? h("span", {
              style: { position: "absolute", right: 6, bottom: 6, padding: "1px 7px", borderRadius: 999, background: "rgba(20,20,22,.55)", color: "#fff", fontSize: 10.5, fontWeight: 600 }
            }, (safe + 1) + "/" + slides.length) : null),

          // 文字块：标题 → 规格 → 小标签 → 分隔线 → 店铺行，四行就是淘宝卡片的
          // 信息层级。整块都可点，省得非要去点图片。
          h("div", {
            onClick: open,
            title: title + " · " + shots.length + " 张成片",
            style: { padding: "8px 9px 9px", cursor: "pointer" }
          },
            h("div", { className: "ecom-clamp2", style: { color: UI.text, fontSize: 12.5, fontWeight: 600, lineHeight: 1.45 } }, title),
            h("div", { style: { display: "flex", alignItems: "baseline", gap: 6, marginTop: 4 } },
              h("span", { style: { color: "#d4380d", fontSize: 14.5, fontWeight: 700 } }, shots.length + " 张成片"),
              // Dated by the newest 成片, not by whatever slide is showing: the date is
              // "when this 款式 was shot", and on the cover slide there is no date at all.
              h("span", { style: { ...UI.muted, fontSize: 11 } }, shots[0] ? fmtStamp(shots[0].createdAt) : "")),
            tags.length > 0 ? h("div", { style: { display: "flex", gap: 4, flexWrap: "wrap", marginTop: 5 } },
              tags.slice(0, 3).map(function (tag, index) {
                return h("span", {
                  key: index,
                  style: { flex: "0 0 auto", padding: "1px 5px", borderRadius: 3, border: "1px solid #f0d6cd", color: "#c1440e", fontSize: 10.5, lineHeight: 1.6 }
                }, tag);
              })) : null,
            card.origin ? h("div", { style: { display: "flex", alignItems: "center", gap: 5, marginTop: 7, paddingTop: 6, borderTop: "1px solid #f2f2f4", minWidth: 0 } },
              h("span", { style: { flex: "0 0 auto", color: "#a9acb3", display: "flex" } }, IconShirt(13)),
              // The 款式 (colour) is the part that distinguishes two cards of the
              // same T恤 — without it six products of one garment read as six
              // copies of the same line.
              h("span", { style: { flex: 1, minWidth: 0, ...UI.muted, fontSize: 11, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } },
                card.origin.colorway ? card.origin.tshirt + " · " + card.origin.colorway : card.origin.tshirt)) : null));
      }

      /**
       * 成品库 — the finished shelf, browsed the way a shopping app is browsed:
       * a **waterfall of 款式**, and a product page when you tap one.
       *
       ...
       * gallery. The feed therefore shows **one card per 款式** — its cover — and
       * the other shots of that 款式 are one swipe away on the card itself.
       */
      function ProductLibrary(props) {
        var openState = React.useState(null); // { groupKey, styleFile, shotId } while a 商品 is open
        var open = openState[0]; var setOpen = openState[1];
        ...
        // Which T恤 and which 款式 a finished shot belongs to, resolved **live**
        // from the T恤 records rather than read off the shot.
        //
        // Two reasons it cannot be read off the shot. Records written before those
        // fields existed carry neither, and a name baked in at generation time goes
        // stale the moment the T恤 or its 款式 is renamed — which is precisely the
        // case where someone is looking at this line to check they renamed the
        // right thing.
        //
        // The path is composite → its 二创T恤 row → the T恤 photo it was composited
        // onto → the 款式 owning that photo. Both steps are indexed once here: a
        // per-card search would be 307 × 99 rows on every render.
        var photoByComposite = {};
        (props.tshirtRecreations || []).forEach(function (row) {
          (row.prints || []).forEach(function (p) { if (p && p.file) photoByComposite[p.file] = row; });
        });
        var ownerByPhoto = {};
        (props.tshirts || []).forEach(function (t) {
          colorwaysOf(t).forEach(function (cw) {
            cw.white.forEach(function (file) {
              ownerByPhoto[file] = { tshirt: t.name, tshirtId: t.id, colorway: cw.name, sizeImages: t.sizeImages || [] };
            });
          });
        });
        /**
         * @returns {{tshirt: string, tshirtId: string|null, colorway: string|null,
         *   sizeImages: string[]}|null} the woven-from garment, or null when there is
         *   nothing truthful to say.
         */
        function originOf(shot) {
          if (!shot) return null;
          var row = photoByComposite[shot.tshirtFile] || null;
          var photo = row ? row.tshirtFile : shot.tshirtPhoto;
          var owner = photo ? ownerByPhoto[photo] : null;
          if (owner) return owner;
          // The composite is gone from 二创T恤 (or predates it). The record's own
          // copy of the name is all that is left; showing it is better than
          // showing nothing, and it is what the shot was actually made from. No id
          // to follow, so the 尺码图 cannot be resolved either.
          var stored = row ? row.tshirtName : shot.tshirtName;
          return stored ? { tshirt: stored, tshirtId: null, colorway: null, sizeImages: [] } : null;
        }

        // 商品 -> 款式 -> that 款式's shots. One card per 款式, newest first,
        // because the feed lists *things*, not individual images.
        var goods = [];
        var byKey = {};
        var cards = [];
        products.forEach(function (p) {
          var groupKey = p.groupKey || "未命名";
          if (!byKey[groupKey]) { byKey[groupKey] = { key: groupKey, name: p.groupName || groupKey, items: [], styles: [], byStyle: {} }; goods.push(byKey[groupKey]); }
          var good = byKey[groupKey];
          good.items.push(p);
          if (!good.byStyle[p.tshirtFile]) {
            var card = {
              key: groupKey + "|" + p.tshirtFile,
              groupKey: groupKey,
              groupName: p.groupName || groupKey,
              styleFile: p.tshirtFile,
              sceneFile: p.sceneFile,
              styleIndex: 0,
              aspect: null,
              ratio: null,
              // 款式是"某张 T恤 照片 × 某张二创印花"，卡片上那行"店铺"位置放的就是
              // 这件 T恤 —— 老记录没有这个字段，缺了就不渲染那一行。
              tshirtName: p.tshirtName || null,
              // …and this is resolved from the live T恤 records, so it covers the
              // records that carry no name at all and follows a rename.
              origin: originOf(p),
              items: []
            };
            good.byStyle[p.tshirtFile] = card;
            good.styles.push(card);
            cards.push(card);
          }
          good.byStyle[p.tshirtFile].items.push(p);
        });
        goods.forEach(function (good) {
          good.items.sort(function (a, b) { return (b.createdAt || 0) - (a.createdAt || 0); });
          good.styles.forEach(function (card, index) {
            card.styleIndex = index;
            card.items.sort(function (a, b) { return (b.createdAt || 0) - (a.createdAt || 0); });
            // 卡片舞台现在是固定的方形，图在里面 contain，所以 swiping 不会改变
            // 卡片高度。测量出的宽高仍留着：比例干净时是卡片上的一个标签，也让
            // 弹框/灯箱知道原图尺寸。
            card.aspect = productAspect(card.items[0]);
            card.ratio = ratioLabel(card.items[0]);
            card.createdAt = card.items[0] ? card.items[0].createdAt : 0;
          });
          good.newest = good.items[0] ? good.items[0].createdAt : 0;
        });
        cards.sort(function (a, b) { return (b.createdAt || 0) - (a.createdAt || 0); });

        var ordered = filter === null ? cards : cards.filter(function (card) { return card.groupKey === filter; });

        // Without IntersectionObserver, render everything rather than hiding
        // cards behind a page size nothing can advance.
        ...
        // The open 商品 is an overlay on top of the feed, not a replacement for
        // it: closing it leaves the shelf exactly where it was.
        var modal = (open !== null && byKey[open.groupKey])
          ? h(ProductModal, {
              // Keyed by 商品: switching to another one must start fresh rather
              // than keep the previous 商品's 款式/shot position.
              key: open.groupKey,
              product: byKey[open.groupKey],
              focusId: open.shotId,
              focusStyleFile: open.styleFile,
              // The modal shows the shot's garment too, and it is the same
              // resolution the card used — done once in the shelf, not twice.
              originOf: originOf,
              onZoom: props.onZoom,
              ...
              distributeColumns(ordered.slice(0, shown), PRODUCT_COLUMNS).map(function (column, columnIndex) {
                return h("div", {
                  ...
                  return h(ProductCard, {
                    key: card.key,
                    card: card,
                    onOpen: function (groupKey, shotId, styleFile) { setOpen({ groupKey: groupKey, shotId: shotId, styleFile: styleFile }); }
                  });
                }));
              })),
        ...
        var chips = h("div", { style: { flex: "0 0 auto", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", margin: "2px 2px 12px" } },
          h("span", { style: { ...UI.muted } }, cards.length + " 个款式" + (hasMore ? " · 已显示 " + shown : "") + (filter !== null ? " · 共 " + ordered.length : "")),
          h("span", { style: { flex: 1 } }),
          ...
          goods.length > 1 ? goods.map(function (good) {
            return h("button", {
              key: good.key,
              onClick: function () { setFilter(good.key); },
              style: Object.assign({}, filter === good.key ? UI.chipOn : UI.chip, { flex: "0 0 auto" })
            }, good.name + " " + good.styles.length);
          }) : null,
        ...
        return h("div", { style: { flex: 1, minHeight: 0, overflow: "hidden", display: "flex", flexDirection: "column" } }, chips, feed, modal);
      }

      /**
       * One 商品, opened as a **modal** over the feed — shaped like a Taobao
       * product page.
       *
       * **Left rail = every 款式 of this 商品** (Taobao's SKU list: one thumbnail
       * per colourway, the selected one outlined). The middle is that 款式's
       * gallery — big image, ‹ ›, a counter, click to zoom — and the right column
       * says what the shot is made of, shows the 款式's whole contact sheet, and
       * lists the product's own numbers. Arrow keys move between shots, Escape or
       * a backdrop click closes it.
       *
       * A modal rather than a page: the shelf is a place you scan, and opening a
       * product should not take the shelf away — you close it and you are still
       * where you were. It sits below the shared lightbox in z-order, so clicking
       * the cover still opens that on top for a true full-screen look.
       *
       * The 款式 strip along the bottom is still deliberately absent — the rail
       * does that job without spending any of the image's height.
       */
      function ProductModal(props) {
        ...
        // Open on the shot the feed was showing, so opening a card lands on that
        // image rather than on the first one of the 款式. With no shot named, it lands
        // on the 产品展示图 — the strip's first entry and the listing's主图.
        var focusStyle = 0;
        var focusShot = 0;
        var focusMatched = false;
        // The card's cover slide (产品展示图) carries no shot id — that is the
        // default slide of every card, so most clicks arrive with no `focusId`
        // at all. Without this, they all fell back to 款式 #1 regardless of
        // which card was actually clicked; matching on the card's own styleFile
        // lands on the right 款式 even when there is no shot to match.
        if (props.focusStyleFile) {
          styles.forEach(function (style, si) { if (style.file === props.focusStyleFile) focusStyle = si; });
        }
        if (props.focusId) {
          styles.forEach(function (style, si) {
            style.items.forEach(function (item, sj) {
              if (item.id === props.focusId) { focusStyle = si; focusShot = sj; focusMatched = true; }
            });
          });
        }

        // The modal opens on the 款式 the feed was showing and stays there until
        // the rail says otherwise — Taobao's product page, where the left rail is
        // the SKU list and the middle is that SKU's gallery. Switching 款式 is why
        // the rail exists at all: without it you would close the modal, find
        // another card, and open that, just to compare two colourways.
        ...
        // Everything one 款式 has, in ONE strip: the 产品展示图 (the 融合 output — the
        // listing's主图), then its 场景成片, then the T恤's 尺码图.
        //
        // The big viewer, the ‹ › arrows, the counter, the keyboard and the strip are
        // all backed by this single list, so "the current image" means one thing
        // rather than three that can disagree.
        //
        // 尺码图 belongs to the T恤 rather than to a colour, so every 款式 of a garment
        // shows the same one — shown, never generated: a size table is a diagram with
        // numbers on it, and a model asked to draw one invents the numbers.
        var currentOrigin = typeof props.originOf === "function" ? props.originOf(current.items[0] || null) : null;
        var sizeImages = (currentOrigin && currentOrigin.sizeImages) || [];
        var shots = [];
        if (current.file) shots.push({ key: "main:" + current.file, file: current.file, kind: "main" });
        current.items.forEach(function (item) {
          shots.push({ key: item.id, file: item.file, kind: "shot", item: item });
        });
        sizeImages.forEach(function (file) { shots.push({ key: "size:" + file, file: file, kind: "size" }); });

        // The feed hands over the id of the shot it was showing, which used to be an
        // index straight into the shots — offset here by the 产品展示图 that leads.
        var shotState = React.useState(focusMatched ? focusShot + (current.file ? 1 : 0) : 0);
        var shotIndex = shotState[0]; var setShotIndex = shotState[1];
        var safeShotIndex = Math.min(Math.max(shotIndex, 0), Math.max(shots.length - 1, 0));
        var shot = shots[safeShotIndex] || null;
        // The product record behind the current slide, or null on the 产品展示图 and the
        // 尺码图 — neither of which is a generated shot, so neither has a scene behind
        // it, a delete of its own, or a creation time.
        var shotItem = shot && shot.item ? shot.item : null;

        // Switching 款式 lands on that 款式's first shot: the two have different
        // counts and different compositions, so keeping the old index would drop
        // the user on an unrelated image.
        function chooseStyle(index) { setStyleIndex(index); setShotIndex(0); }
        ...
        React.useEffect(function () {
          ...
          function onKey(event) {
            if (event.key === "Escape") {
              // The lightbox sits on top of this modal and closes itself on
              // Escape too; closing both at once would throw away the product
              // the user was looking at.
              if (document.querySelector("[data-ecom-lightbox]")) return;
              props.onClose();
              return;
            }
        ...
        var arrow = function (side) {
          return {
            ...
            border: "1px solid rgba(255,255,255,.6)", background: "rgba(20,20,22,.45)", color: "#fff",
            cursor: shots.length < 2 ? "default" : "pointer", opacity: shots.length < 2 ? 0.35 : 1,
        ...
        return h("div", {
          ...
          style: {
            ...
            background: "rgba(20,20,22,0.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24
          }
        },
          h("div", {
            onClick: function (e) { e.stopPropagation(); },
            style: {
              width: "min(1120px, 96vw)", height: "min(780px, 90vh)", background: "#ffffff",
              borderRadius: 16, boxShadow: "0 24px 70px rgba(0,0,0,.45)",
              display: "flex", flexDirection: "column", padding: 16, overflow: "hidden"
            }
          },
            h("div", { style: { flex: "0 0 auto", display: "flex", alignItems: "center", gap: 12, marginBottom: 12 } },
              ...
              h("span", { style: { flex: 1 } }),
              // 删除 only applies to a generated shot: the 产品展示图 and the 尺码图 are
              // not shots of this 商品 and deleting them here would delete an upload.
              shotItem ? h("button", {
                onClick: function () { props.onDelete(shotItem.id); },
                style: Object.assign({}, UI.btnGhost, { padding: "5px 10px", fontSize: 12.5, color: "#b23c2e" })
              }, "删除这张") : null,
              h("button", {
                onClick: props.onClose, title: "关闭（Esc）",
                style: { flex: "0 0 auto", width: 30, height: 30, padding: 0, borderRadius: 9, border: "1px solid #e5e5ea", background: "#f5f5f7", color: UI.text2, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }
              }, IconClose(15))),

            // Everything below shares the leftover height; each column scrolls
            // itself rather than making the panel scroll.
            h("div", { style: { flex: 1, minHeight: 0, display: "flex", gap: 12 } },
              // Left rail: **every 款式 of this 商品**, the way Taobao's product
              // page lists the colourways. One tap switches the big image to that
              // 款式's gallery without leaving the product.
              h("div", { style: { flex: "0 0 76px", minHeight: 0, display: "flex", flexDirection: "column", gap: 6, overflowY: "auto", overflowX: "hidden", paddingRight: 2 } },
                styles.map(function (style, index) {
                  return h("button", {
                    key: style.file || index,
                    onClick: function () { chooseStyle(index); },
                    title: "款式 #" + (index + 1) + " · " + style.items.length + " 张成片",
                    style: { flex: "0 0 auto", padding: 0, borderRadius: 10, overflow: "hidden", cursor: "pointer", background: "#f5f5f7", border: "2px solid " + (index === safeStyleIndex ? "#1c1d1f" : "transparent") }
                  }, h("img", { src: fileUrl(style.file), loading: "lazy", alt: "", style: { width: "100%", aspectRatio: "3 / 4", objectFit: "cover", display: "block" } }));
                })),

              h("div", { style: { flex: 1, minWidth: 0, minHeight: 0, display: "flex", flexDirection: "column", gap: 8 } },
                h("div", { style: { flex: 1, minHeight: 0, position: "relative", borderRadius: 14, background: "#f5f5f7", border: "1px solid #ececf0", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" } },
                  shot
                    ? h("img", {
                        ...
                        // Never larger than the stage, and never stretched: the
                        // height chain down to here is definite, so contain
                        // scales the shot down to fit instead of overflowing.
                        style: { maxWidth: "100%", maxHeight: "100%", width: "auto", height: "auto", objectFit: "contain", cursor: "zoom-in", display: "block" }
                      })
                    : h("span", { style: { ...UI.muted } }, "这个款式还没有成片"),
                  shots.length > 1 ? h("button", { title: "上一张", onClick: function () { step(-1); }, style: arrow("left") }, "‹") : null,
                  shots.length > 1 ? h("button", { title: "下一张", onClick: function () { step(1); }, style: arrow("right") }, "›") : null),
                h("div", { style: { flex: "0 0 auto", display: "flex", alignItems: "center", gap: 10, ...UI.muted } },
                  h("span", null, shots.length > 0 ? "第 " + (safeShotIndex + 1) + " / " + shots.length + " 张" : "—"),
                  h("span", { style: { flex: 1 } }),
                  h("span", null, "点击图片可全屏放大")),

                // The strip: everything this 款式 has, in the order a listing is
                // assembled — 产品展示图 first (it is the 主图), then the 成片, then the
                // 尺码图. Clicking one shows it in the stage above, and the ‹ › arrows
                // and the counter walk this same list, so there is one notion of "the
                // current image" rather than a gallery that disagrees with a panel.
                //
                // `contain`, not `cover`: a 尺码图 cropped to a 3:4 box cuts the table,
                // and the 成片 are 3:4 anyway so nothing is lost.
                shots.length > 0 ? h("div", { style: { flex: "0 0 auto", display: "flex", gap: 6, overflowX: "auto", overflowY: "hidden", paddingBottom: 2 } },
                  shots.map(function (entry, index) {
                    return h("button", {
                      key: entry.key,
                      onClick: function () { setShotIndex(index); },
                      title: entry.kind === "main" ? "产品展示图" : (entry.kind === "size" ? "尺码图" : "成片（第 " + (index + 1) + " 张）"),
                      style: { flex: "0 0 auto", width: 54, padding: 0, borderRadius: 8, overflow: "hidden", cursor: "pointer", background: "#fff", border: "2px solid " + (index === safeShotIndex ? "#1c1d1f" : "#ececf0") }
                    }, h("img", { src: fileUrl(entry.file), loading: "lazy", alt: "", style: { width: "100%", aspectRatio: "3 / 4", objectFit: "contain", display: "block" } }));
                  })) : null),

              // What this shot is made of, and what the product is.
              h("div", { style: { flex: "0 0 210px", minHeight: 0, display: "flex", flexDirection: "column", gap: 10, overflowY: "auto" } },
                h("div", { style: { ...UI.panel, padding: 12 } },
                  h("div", { style: { color: UI.text, fontSize: 12.5, fontWeight: 600, marginBottom: 8 } }, "这一张的来源"),
                  h("div", { style: { display: "flex", gap: 8 } },
                    shotItem && shotItem.sceneFile ? h("div", { style: { flex: 1, minWidth: 0 } },
                      h("img", { src: fileUrl(shotItem.sceneFile), loading: "lazy", alt: "", style: { width: "100%", aspectRatio: "1 / 1", objectFit: "cover", borderRadius: 8, border: "1px solid #ececf0", display: "block" } }),
                  ...
                  (function () {
                    // Which garment and colour this shot is of. Resolved the same way
                    // the card resolves it, so the two can never disagree. On the
                    // 产品展示图 and the 尺码图 there is no shot, so it falls back to the
                    // 款式 itself — the garment line stays truthful on every slide.
                    var origin = typeof props.originOf === "function" ? props.originOf(shotItem || current.items[0] || null) : null;
                    if (origin === null) return null;
                    return h("div", { style: { display: "flex", alignItems: "center", gap: 5, marginTop: 10, paddingTop: 8, borderTop: "1px solid #f2f2f4", minWidth: 0 } },
                      h("span", { style: { flex: "0 0 auto", color: "#a9acb3", display: "flex" } }, IconShirt(13)),
                      h("span", { style: { flex: 1, minWidth: 0, color: UI.text2, fontSize: 11.5, lineHeight: 1.5 } },
                        origin.colorway ? origin.tshirt + " · " + origin.colorway : origin.tshirt));
                  })(),
                  h("button", {
                ...
                h("div", { style: { ...UI.panel, padding: 12 } },
                  ...
                  h("div", { style: { fontSize: 12, color: UI.text2, lineHeight: 1.9 } },
                    ...
                    h("div", null, "该款式成片：" + current.items.length + " 张"),
                    h("div", null, "全部成片：" + product.items.length + " 张"),
                    // Only a generated shot has a creation time; the 产品展示图 and the
                    // 尺码图 are not shots and would print a blank.
                    shotItem ? h("div", null, "生成于 " + fmtStamp(shotItem.createdAt)) : null))))));
      }
      ...
      function Workbench() {
        ...
        var viewEls = [
          ...
          h(WorkflowManager, {
            workflows: workflows,
            // 印花流水线的分组行要能选「这一组跑哪件 T恤」，选择器读的就是
            // 这一份列表；它已经加载好了，没必要在读工作流时再取一次。
            tshirts: tshirts,
            refresh: fetchState,
            ...
            // Threaded down to the pipeline's group composer, whose paste listener
            // must not fire while another view is on screen — every view stays
            // mounted (hidden with `display:none`), so without this an image pasted
            // into 印花提取 would silently land in the pipeline's group instead.
            active: view === "工作流",
          ...
          h(ProductLibrary, { onZoom: setLightboxImage, tshirts: tshirts, tshirtRecreations: tshirtRecreations }),
      ...
      function TshirtManager(props) {
        ...
        /**
         * Add photos to one bucket of one T恤.
         *
         * `key` identifies the bucket for the busy spinner and the hidden input, so
         * a slow upload into 白色's 细节图 cannot make 黑色's 白底图 look busy.
         *
         * The reply replaces the record wholesale rather than patching a field: the
         * host normalizes on the way out, and the client's copy carries the same
         * derived `images` list, so a local patch would put the two out of step.
         *
         * `slot` is "white" / "detail" for a 款式's buckets, or "size" for the
         * product's 尺码图 — which is the one bucket with no colorwayId.
         */
        function onPickBucketFiles(key, tshirtId, colorwayId, slot, e) {
        ...
        function addColorway(tshirtId) {
          setError("");
          apiPost("/tshirt/addColorway", { id: tshirtId }).then(function (res) {
            if (!res || res.ok !== true) { setError((res && res.error) || "添加款式失败"); return; }
            setTshirts(function (prev) { return prev.map(function (t) { return t.id === res.tshirt.id ? res.tshirt : t; }); });
          }, function (err) { setError(String((err && err.message) || err)); });
        }
        /**
         * Rename the product. The 款式 rename two lines down is the same route with
         * a colorwayId — without one the host renames the T恤 itself, which is how
         * a name typed at 创建T恤 time gets corrected.
         */
        function renameTshirt(tshirtId, current) {
          var next = promptAction("这件T恤叫什么？", current);
          if (next === null || next.trim() === "" || next.trim() === current) return;
          apiPost("/tshirt/rename", { id: tshirtId, name: next.trim() }).then(function (res) {
            if (!res || res.ok !== true) { setError((res && res.error) || "改名失败"); return; }
            setTshirts(function (prev) { return prev.map(function (t) { return t.id === res.tshirt.id ? res.tshirt : t; }); });
          }, function (err) { setError(String((err && err.message) || err)); });
        }
        function renameColorway(tshirtId, colorwayId, current) {
          var next = promptAction("这个款式叫什么？（例如「白色」「杏色」）", current);
          if (next === null || next.trim() === "" || next.trim() === current) return;
          apiPost("/tshirt/rename", { id: tshirtId, colorwayId: colorwayId, name: next.trim() }).then(function (res) {
            if (!res || res.ok !== true) { setError((res && res.error) || "改名失败"); return; }
            setTshirts(function (prev) { return prev.map(function (t) { return t.id === res.tshirt.id ? res.tshirt : t; }); });
          }, function (err) { setError(String((err && err.message) || err)); });
        }
        function removeColorway(tshirtId, colorwayId, name) {
          if (!confirmAction("确定删除款式「" + name + "」？它的白底图和细节图都会一起删掉。")) return;
          setTshirts(function (prev) {
            return prev.map(function (t) {
              if (t.id !== tshirtId) return t;
              var kept = colorwaysOf(t).filter(function (c) { return c.id !== colorwayId; });
              var images = [];
              kept.forEach(function (c) { c.white.forEach(function (f) { images.push(f); }); });
              return Object.assign({}, t, { colorways: kept, images: images });
            });
          });
          apiPost("/delete", { kind: "tshirtColorway", id: tshirtId, colorwayId: colorwayId });
        }
        ...
        function removeImage(tshirtId, file) {
          ...
          // Edited through the 款式 it lives in, then `images` is rebuilt from them:
          // it is a derived field, so filtering it directly would leave the record
          // claiming a photo it no longer has.
          setTshirts(function (prev) {
            return prev.map(function (t) {
              if (t.id !== tshirtId) return t;
              var colorways = colorwaysOf(t).map(function (c) {
                return Object.assign({}, c, {
                  white: c.white.filter(function (f) { return f !== file; }),
                  detail: c.detail.filter(function (f) { return f !== file; })
                });
              });
              var images = [];
              colorways.forEach(function (c) { c.white.forEach(function (f) { images.push(f); }); });
              return Object.assign({}, t, {
                colorways: colorways,
                images: images,
                sizeImages: (t.sizeImages || []).filter(function (f) { return f !== file; })
              });
            });
          });
        ...
        /**
         * One bucket: a 款式's 白底图 or 细节图, or the product's 尺码图.
         *
         * Its own labelled row, because the point of the grouping is that these are
         * different things — a 白底图 becomes a product shot, a 细节图 is evidence
         * the model gets, a 尺码图 is a listing asset. One undifferentiated pile is
         * what this replaces.
         *
         * `key` identifies the bucket for the busy spinner and the hidden file
         * input, so a slow upload into 白色's 细节图 cannot make 黑色's 白底图 look
         * busy.
         */
        function bucket(tshirt, colorwayId, slot, files, label) {
          var key = tshirt.id + "|" + (colorwayId || "size") + "|" + slot;
          var adding = addBusyId === key;
          var small = { flex: "0 0 auto", width: 22, height: 22, borderRadius: 7, border: "1px solid #e5e5ea", background: "#fff", color: UI.text2, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", justifyContent: "center" };
          return h("div", { style: { display: "flex", flexDirection: "column", gap: 6 } },
            h("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
              h("span", { style: { fontSize: 11.5, fontWeight: 600, color: UI.text2 } }, label),
              h("span", { style: { ...UI.muted, fontSize: 11 } }, files.length > 0 ? files.length + " 张" : "还没有"),
              h("span", { style: { flex: 1 } }),
              h("input", {
                ref: function (el) { addInputRefs.current[key] = el; },
                type: "file", accept: "image/*", multiple: true,
                onChange: function (e) { onPickBucketFiles(key, tshirt.id, colorwayId, slot, e); },
                style: { display: "none" }
              }),
              h("button", {
                title: "添加" + label, disabled: adding,
                onClick: function () { var el = addInputRefs.current[key]; if (el) el.click(); },
                style: adding ? Object.assign({}, small, { cursor: "default" }) : Object.assign({}, small, { border: "1px dashed #d8d8dd", background: "#fafafb" })
              }, adding ? Spinner() : IconPlus(11))),
            files.length > 0 ? h("div", { style: { display: "flex", flexWrap: "wrap", gap: 6 } },
              files.map(function (file) {
                return h("div", { key: file, style: { position: "relative" } },
                  Thumb({ src: fileUrl(file), size: 52, label: tshirt.name + " " + label, onZoom: onZoom }),
                  h("button", { title: "移除", onClick: function () { removeImage(tshirt.id, file); }, style: { position: "absolute", top: -6, right: -6, width: 17, height: 17, borderRadius: 999, border: "1px solid #e5e5ea", background: "#fff", color: "#9aa0a8", cursor: "pointer", padding: 0, display: "flex", alignItems: "center", justifyContent: "center" } }, IconClose(9)));
              })) : null);
        }

        /**
         * One 款式 (colour): its name, and the two buckets that belong to it.
         *
         * The 细节图 live inside this block rather than in a separate list because
         * they belong to this colour — that ownership is the whole point. Picking
         * the 款式 downstream is what brings them along.
         */
        function colorwayBlock(t, cw) {
          var tiny = { flex: "0 0 auto", padding: "2px 7px", borderRadius: 7, border: "1px solid #e5e5ea", background: "#fff", color: UI.text2, cursor: "pointer", fontSize: 11, display: "flex", alignItems: "center", gap: 3 };
          return h("div", { key: cw.id, style: { border: "1px solid #ececf0", borderRadius: 10, padding: "8px 9px", display: "flex", flexDirection: "column", gap: 9, background: "#fcfcfd" } },
            h("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
              h("span", { style: { fontSize: 12.5, fontWeight: 700, color: UI.text, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, cw.name),
              h("span", { style: { flex: 1 } }),
              h("button", { title: "给这个款式改名", onClick: function () { renameColorway(t.id, cw.id, cw.name); }, style: tiny }, "改名"),
              h("button", { title: "删除这个款式（连它的图片一起）", onClick: function () { removeColorway(t.id, cw.id, cw.name); }, style: Object.assign({}, tiny, { padding: "2px 5px" }) }, IconTrash(11))),
            bucket(t, cw.id, "white", cw.white, "白底图"),
            bucket(t, cw.id, "detail", cw.detail, "细节图"),
            // A shot of the colour on a body. Kept out of 白底图 because nothing
            // generates onto it — it is a listing asset, and folding it in would
            // make the pipeline render one product per model shot.
            bucket(t, cw.id, "model", cw.model || [], "模特上身图"));
        }

        var grid;
        if (tshirts.length > 0) {
          grid = h("div", { style: { flex: 1, minHeight: 0, overflow: "auto", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 12, padding: "4px 4px 12px", alignContent: "start" } },
            tshirts.slice().sort(byNewest).map(function (t) {
              var list = colorwaysOf(t);
              var whiteCount = 0;
              list.forEach(function (c) { whiteCount += c.white.length; });
              return h("div", { key: t.id, style: { background: "#ffffff", border: "1px solid #ececf0", borderRadius: 14, padding: 12, display: "flex", flexDirection: "column", gap: 10 } },
                h("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
                  // The name is what the group picker and the product records quote,
                  // so it has to be correctable after 创建T恤 — a name typed in a
                  // hurry is otherwise permanent, and the wrong one follows every
                  // 款式 and every finished product.
                  h("div", { style: { fontSize: 13.5, fontWeight: 600, color: UI.text, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, t.name),
                  h("button", { title: "给这件T恤改名", onClick: function () { renameTshirt(t.id, t.name); }, style: { flex: "0 0 auto", padding: "2px 7px", borderRadius: 7, border: "1px solid #e5e5ea", background: "#fff", color: UI.text2, cursor: "pointer", fontSize: 11 } }, "改名"),
                ...
                list.map(function (cw) { return colorwayBlock(t, cw); }),
                h("button", {
                  onClick: function () { addColorway(t.id); },
                  style: { display: "flex", alignItems: "center", justifyContent: "center", gap: 4, padding: "7px 0", borderRadius: 10, border: "1px dashed #d8d8dd", background: "#fafafb", color: UI.text2, cursor: "pointer", fontSize: 12 }
                }, IconPlus(12), "添加款式"),
                // 尺码图 sit outside the 款式 list on purpose: one pattern means one
                // size chart, so repeating it per colour would be four copies of the
                // same table.
                h("div", { style: { borderTop: "1px solid #ececf0", paddingTop: 9 } },
                  bucket(t, null, "size", Array.isArray(t.sizeImages) ? t.sizeImages : [], "尺码图")),
                h("div", { style: { ...UI.muted } }, list.length + " 个款式 · " + whiteCount + " 张白底图"));
            }));
        } else if (!creating) {
      ...
      // ---------- 收起输入框（shell overlay） --------------------------------
      /** Where "the composer is collapsed" is remembered, across reloads. */
      var COMPOSER_COLLAPSED_KEY = "dsh-ecom-composer-collapsed";

      function readComposerCollapsed() {
        try {
          return typeof localStorage !== "undefined" && localStorage.getItem(COMPOSER_COLLAPSED_KEY) === "1";
        } catch (error) {
          return false;
        }
      }
      function writeComposerCollapsed(value) {
        try {
          if (typeof localStorage !== "undefined") localStorage.setItem(COMPOSER_COLLAPSED_KEY, value ? "1" : "0");
        } catch (error) {
      ...
      /**
       * A frame-wide pill that collapses the conversation composer, giving the
       * workbench the height back.
       *
       * **Why `shell.overlay`.** The composer belongs to the shell, not to this
       * plugin, and once it is collapsed the only way back has to be something
       * that is always on screen — a button inside the workbench would vanish
       * with the view that needs it. The shell documents that seat as a
       * "frame-wide floating layer, above every column and outside their scroll
       * containers", additive (`replaceRisk: none`) and click-through per entry,
       * so this button can neither shadow nor block anything else.
       *
       * **How the composer is collapsed.** The conversation UI marks the
       * composer's seat with `data-composer-seat` — a hook it declares itself,
       * not a hashed CSS class — and React never sets a `style` on that element
       * (it gets `className` and a ref only), so `display` is the plugin's to
       * take and to put back. This is the same reach into shell DOM that the
       * layout binding already does; it is deliberately the *only* element this
       * plugin touches that it does not own, and if the attribute ever goes away
       * the toggle does nothing instead of breaking the shell.
       */
      function ComposerToggle() {
        var collapsedState = React.useState(readComposerCollapsed);
        var collapsed = collapsedState[0]; var setCollapsed = collapsedState[1];
        var posState = React.useState({ right: 22, bottom: 14 });
        var pos = posState[0]; var setPos = posState[1];
        // Kept in a ref, not state: it is a measurement, and re-rendering because
        // a number was re-read is how a layout effect starts looping.
        var rightRef = React.useRef(22);

        React.useEffect(function () {
          if (typeof document === "undefined") return undefined;
          function seat() { return document.querySelector("[data-composer-seat]"); }
          function layer() { return document.querySelector("[data-shell-overlay]"); }
          ...
          function measure() {
            ...
            if (seatRect.height > 0) rightRef.current = Math.max(12, Math.round(boxRect.right - seatRect.right + 8));
            var next = {
              right: rightRef.current,
              // 展开时贴在输入框上沿；收起后座位没有高度，"上沿"本来就等于底边，
              // 所以同一个公式两种情况都成立。
              bottom: seatRect.height > 0 ? Math.max(8, Math.round(boxRect.bottom - seatRect.top + 10)) : 14
            };
          ...
          /** Make the seat's display match the remembered preference. */
          function apply() {
            var el = seat();
            if (!el) return;
            if (collapsed) {
              if (el.dataset.ecomComposerCollapsed !== "1") {
                el.dataset.ecomComposerCollapsed = "1";
                el.dataset.ecomPrevDisplay = el.style.display || "";
                el.style.display = "none";
              }
            } else if (el.dataset.ecomComposerCollapsed === "1") {
              el.style.display = el.dataset.ecomPrevDisplay || "";
              delete el.dataset.ecomComposerCollapsed;
              delete el.dataset.ecomPrevDisplay;
            }
            measure();
          }

          apply();
          // Expanded needs no upkeep — the seat is there by itself. Collapsed has
          // to survive the shell rebuilding that element (a session bind does),
          // so it re-asserts at a low frequency, and only while collapsed.
          var timer = collapsed ? setInterval(apply, 1200) : null;
          window.addEventListener("resize", apply);

          // A blank session centres the composer (phase `hero`) and the first
          // message sends it to the bottom, which moves it without resizing
          // anything — so position is re-read when the shell's own phase
          // attribute changes, and not on every DOM mutation.
          var phaseWatcher = null;
          if (typeof MutationObserver !== "undefined") {
            phaseWatcher = new MutationObserver(measure);
            phaseWatcher.observe(document.body, { attributes: true, attributeFilter: ["data-phase"], subtree: true });
          }

          return function () {
            if (timer !== null) clearInterval(timer);
            window.removeEventListener("resize", apply);
            if (phaseWatcher !== null) phaseWatcher.disconnect();
          };
        }, [collapsed]);

        // Give the seat back when this component goes away (plugin disabled, hot
        // reload): leaving the shell with no input box and no button is the one
        // outcome worth guarding against.
        React.useEffect(function () {
          return function () {
            if (typeof document === "undefined") return;
            var el = document.querySelector("[data-composer-seat]");
            if (el && el.dataset.ecomComposerCollapsed === "1") {
              el.style.display = el.dataset.ecomPrevDisplay || "";
              delete el.dataset.ecomComposerCollapsed;
              delete el.dataset.ecomPrevDisplay;
            }
        ...
        return h("div", { style: { position: "absolute", right: pos.right, bottom: pos.bottom } },
          h("button", {
            onClick: function () {
              var next = !collapsed;
              setCollapsed(next);
              writeComposerCollapsed(next);
            },
            title: collapsed ? "展开对话输入框" : "收起对话输入框，把这块高度让给工作台",
            style: {
              display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px",
              borderRadius: 999, border: "1px solid #e5e5ea", background: "rgba(255,255,255,.94)",
              color: UI.text2, cursor: "pointer", font: "inherit", fontSize: 11.5, lineHeight: 1.6,
              boxShadow: "0 2px 10px rgba(20,20,25,.10)"
            }
          }, collapsed ? "⌃ 展开输入框" : "⌄ 收起输入框"));
      }

      slots.inject("shell.overlay", function () {
        // list 插槽：一个新 id 加在既有条目旁边，不替换任何东西。order 取 100，
        // 让别的浮层（提示条之类）排在这个小按钮前面。
        return slots.register({
          name: "shell.overlay",
          id: "ecom-composer-toggle",
          order: 100,
          label: function () { return "收起输入框"; }
        }, ComposerToggle);
      });
