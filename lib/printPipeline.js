...
const printPipeline = {
  ...
  async run(ctx) {
    ...
    if (allTshirtRows.length > 0 && plan.prompts.scene.found && plan.scenes.available > 0) {
      ...
      // Each 款式 is shot in `sceneCount` DIFFERENT scenes, drawn at random from
      // the pool without replacement, and every shot of one scene shares it — the
      // sceneOutputs images of a scene are one "generation" of that scene.
      ...
      function scenesForTshirt(tshirtFile) {
        ...
        const chosen = [];
        for (let i = 0; i < sceneCount && pool.length > 0; i++) {
