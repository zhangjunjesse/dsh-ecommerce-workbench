...
// The pipeline normalizes each T恤 itself rather than trusting the caller: it is
// driven by tests and by hand-built states as well as by the store, and a record
// that predates 款式 has to resolve to the same shots in all three.
const { normalizeTshirt } = require("./store.js");
// Required from the settings module, NOT from ./workflows.js: the registry
// requires this file, so reaching back into it would be a cycle — and the
// definition would end up with `effectiveSettings === undefined` at run time.
const { effectiveSettings } = require("./workflowSettings.js");
...
/**
 * Used only when 提示词管理 has no 「印花提取」; the other three are required for
 * their step.
 *
 * Says "这件衣服", not "T恤": the workbench's own module is called T恤管理, but the
 * thing on the other end of this call is whatever the group's screenshots show —
 * on this machine that is 卫衣, 连帽卫衣, 长袖T, 无袖背心, V领无袖 as well as tees, and
 * an instruction to take the print off a T-shirt is simply the wrong instruction
 * for half of them.
 */
const FALLBACK_EXTRACT_PROMPT = "提取这件衣服上的印花；";
...
/**
 * The knobs the user may turn, declared rather than hard-coded.
 *
 * These two ARE the shape of the last step — how many scenes each T恤 is shown in,
 * and how many shots each of those scenes yields — and they are also the two
 * numbers that decide what a run costs. Making them settings (validated, stored,
 * shown in the UI) is the difference between "change the env var and restart the
 * host" and "type a number". The env vars remain the *default* so an existing
 * deployment keeps its configured shape.
 */
const PIPELINE_SETTINGS = [
  {
    ...
    max: 24,
    step: 1,
    default: positiveInt(process.env.ECOM_PIPELINE_SCENE_PASSES, 2),
    help: "每件二创T恤随机挑这么多张场景图（同一件里不重复）。池子不够时按池子大小来。"
  },
  {
    key: "sceneOutputs",
    label: "每个场景图出几张",
    ...
    max: 12,
    step: 1,
    default: positiveInt(process.env.ECOM_PIPELINE_SCENE_OUTPUTS, 4),
    help: "每张场景图生成几张成片。"
  }
];
...
/**
 * A positive pixel dimension, or null.
 *
 * The 成品库 lays its tiles out as a waterfall, which can only reserve a tile's
 * height when the real ratio is known. Measured from the bytes we just wrote
 * (never guessed): a guessed ratio is what stretches an image, and an unknown one
 * simply lays out at its natural size.
 */
...
/**
 * Which T恤 photos this group generates against, and which 款式 they came from.
 *
 * The unit of selection is the **款式** (one colour). Picking 白色T恤 means its
 * 白底图 — front and back — and, downstream, its 细节图; the product's 尺码图 ride
 * along for the listing rather than for generation.
 *
 * The stored selection is filtered against the T恤's real 款式 rather than
 * trusted, so a stale choice can never point the pipeline at a photo that is
 * gone; if nothing survives, it falls back to the documented default (every
 * 款式) and says so. Degrading to "generate nothing" would look like a broken
 * workflow; degrading to the default is what the user would have got had they
 * never picked.
 *
 * `tshirtImages` — the per-photo selection from before 款式 existed — is still
 * honoured, read as "the 款式 that own those photos", so a group configured back
 * then keeps generating exactly the shots it used to.
 *
 * The record is put through the store's normalizer first: the pipeline is also
 * driven by tests and by callers that hand it a hand-built state, and a T恤 that
 * predates 款式 must resolve to the same shots here as it does after a read.
 */
function resolveTshirt(state, config) {
  const tshirts = state.tshirts || [];
  const raw = config.tshirtId
    ? tshirts.filter(function (t) { return t.id === config.tshirtId; })[0]
    : tshirts[0];
  if (!raw) return null;
  const chosen = normalizeTshirt(raw);
  const colorways = chosen.colorways;
  const all = chosen.images;

  const pickedColorways = Array.isArray(config.tshirtColorways) ? config.tshirtColorways : null;
  const pickedImages = Array.isArray(config.tshirtImages) ? config.tshirtImages : null;
  const wantsColorways = pickedColorways !== null && pickedColorways.length > 0;
  const wantsImages = pickedImages !== null && pickedImages.length > 0;

  let selectedColorways;
  if (wantsColorways) {
    selectedColorways = colorways.filter(function (c) { return pickedColorways.indexOf(c.id) !== -1; });
  } else if (wantsImages) {
    selectedColorways = colorways.filter(function (c) {
      return c.white.some(function (f) { return pickedImages.indexOf(f) !== -1; });
    });
  } else {
    selectedColorways = colorways.slice();
  }
  const stale = (wantsColorways || wantsImages) && selectedColorways.length === 0;
  if (stale) selectedColorways = colorways.slice();

  // Every 白底图 of every selected 款式: one composite each, so a colour shot from
  // the front and the back is two products rather than one.
  // A 款式 with no 白底图 cannot be rendered: the composite is a product shot, and a
  // 款式 holding only 细节图 or 模特图 has no shot to draw on. Dropping them here
  // rather than at the fan-out keeps the estimate and the run saying the same
  // number — a count that included them would be a price for work never done.
  const compositable = selectedColorways.filter(function (c) { return c.white.length > 0; });
  const selected = [];
  compositable.forEach(function (c) { selected.push.apply(selected, c.white); });

  return {
    id: chosen.id,
    name: chosen.name || "T恤",
    photos: all,
    // Flattened 白底图, kept for the readers that only want the photos. The step
    // fans out over `colorways`, not over this: one 融合 per colour.
    selected: selected,
    // Each 款式 whole — white, detail and model — because all of it goes to the
    // model as references.
    colorways: compositable.map(function (c) {
      return { id: c.id, name: c.name, white: c.white.slice(), detail: c.detail.slice(), model: c.model.slice() };
    }),
    sizeImages: chosen.sizeImages.slice(),
...
function sceneKey(tshirtFile, sceneIndex, variant) {
  return tshirtFile + "|" + sceneIndex + "|" + variant;
}
...
/**
 * The identity of one 融合 (a 款式 × a 印花).
 *
 * Keyed by the 款式, not by a photo of it: the step renders the whole 款式 and the
 * model picks which shot to draw, so there is no single photo that identifies the
 * result. Keys written by the previous per-photo scheme therefore never match —
 * which is correct, because those composites were made a different way and have
 * to be regenerated rather than counted as done.
 */
function colorwayKey(colorwayId, printFile) {
  return "cw:" + colorwayId + "|" + printFile;
}
...
async function existingIndex(store, groupKey) {
  ...
  return {
    ...
    // A row written by the current scheme carries its 款式; one from the old
    // per-photo scheme does not, and keeps its old-style key so it simply never
    // matches — those composites were made a different way and must be remade.
    tshirtKeys: new Set(tshirts.map(function (row) {
      return row.colorwayId ? colorwayKey(row.colorwayId, row.printFile) : tshirtKey(row.tshirtFile, row.printFile);
    })),
    tshirtRows: tshirts,
    sceneKeys: new Set(products.map(function (row) { return sceneKey(row.tshirtFile, row.sceneIndex, row.variant); })),
    products: products
  };
}
 ...
 */
async function planGroup(store, groupKey, settingsOverride) {
  ...
  // The run passes the values it already resolved, so the plan it checks and the
  // work it does cannot disagree; the estimate endpoint resolves them itself —
  // from the WORKFLOW's config, not the group's (the group record holds the
  // group's own choices; the settings live on the workflow).
  const workflowConfig = (state.workflows || []).filter(function (w) { return w.id === WORKFLOW_ID; })[0] || {};
  const settings = settingsOverride || effectiveSettings(printPipeline, workflowConfig.settings);
  // You cannot shoot a T恤 in more distinct scenes than the pool holds. Asking for
  // more is capped rather than silently repeating a scene.
  const sceneCount = Math.min(settings.sceneCount, Math.max(scenes.length, 0));
  const sceneOutputs = settings.sceneOutputs;
  ...
  else if (settings.sceneCount > scenes.length) {
    warnings.push("参数里选了 " + settings.sceneCount + " 张场景图，但池子里只有 " + scenes.length + " 张，本次按 " + scenes.length + " 张处理（不会重复用同一张）。");
  }
  ...
  // One composite per (印花 × 款式) — the whole 款式 is the input, not one photo of it.
  const tshirtCount = canTshirt ? recreateCount * tshirt.colorways.length : 0;
  const sceneTotal = canScene ? tshirtCount * sceneCount * sceneOutputs : 0;
  ...
  const missingTshirt = canTshirt
    ? Math.max(0, recreateCount * tshirt.colorways.length - existing.tshirtKeys.size)
    : 0;
  const missingScene = canScene
    ? Math.max(0, tshirtCount * sceneCount * sceneOutputs - existing.sceneKeys.size)
    : 0;
  const missingExtract = existing.extract ? 0 : 1;

  const total = (existing.extract ? 0 : 1) + recreateCount + tshirtCount + sceneTotal;
  ...
  return {
    ...
    tshirt: tshirt === null ? null : {
      ...
      // The 款式 the selection resolved to, so the estimate and the run agree on
      // what "选了 N 个款式" means and the 融合 step can find each one's 细节图.
      colorways: tshirt.colorways,
      sizeImages: tshirt.sizeImages,
      selectionStale: tshirt.selectionStale
    },
    scenes: { available: scenes.length },
    plan: { extract: 1, recreate: recreateCount, tshirt: tshirtCount, scene: sceneTotal, total: total },
    ...
    // The effective knobs this plan was computed with — including the cap when the
    // scene pool is smaller than the requested count, so the number shown is the
    // number that will actually be produced.
    settings: {
      recreateOutputs: RECREATE_OUTPUTS,
      sceneCount: sceneCount,
      sceneOutputs: sceneOutputs,
      sceneCountRequested: settings.sceneCount,
  ...
  async run(ctx) {
    ...
    if (groupKey === null) {
      ...
      if (next === null) {
        ...
        const message = "没有已确认、还没跑完的分组，本次没有处理任何东西：请到「分组与队列」里选一组点「估算并运行」，" +
          "或先点那一行上的「排队」把它交给周期执行。";
    ...
    const plan = await planGroup(store, groupKey, ctx.settings);
    ctx.log("分组「" + plan.group.name + "」共 " + plan.group.imageCount + " 张参考图");
    ctx.log("参数：每件二创T恤 " + plan.settings.sceneCount + " 张场景图 × 每张 " + plan.settings.sceneOutputs + " 张成片");
    ...
    // ---- 第三步：T恤融合 ---------------------------------------------------
    const tshirtRows = [];
    if (recreateRows.length > 0 && plan.tshirt && plan.tshirt.colorways.length > 0 && plan.prompts.tshirt.found) {
      // One composite per (印花 × 款式). The whole 款式 goes in and **the model finds
      // the front itself**.
      //
      // That is deliberate, and it is what replaced picking a photo here. A 款式's
      // 白底图 are front, back and further angles in whatever order they were
      // uploaded — on this machine one 款式 is literally 背面, 正面, 正面 — and
      // nothing in the data says which is which. Anything the pipeline picked as
      // "the front" would be a guess, and the guess it used to make (loop over
      // every 白底图) stamped the print onto the back as well.
      const pairs = [];
      for (const print of recreateRows) {
        for (const colorway of plan.tshirt.colorways) {
          pairs.push({
            print: print,
            colorway: colorway,
            // The 白底图 that stands for this 款式 in the records. NOT a canvas —
            // the references are the whole 款式 — but the shelf's composite →
            // 二创T恤 → 白底图 → 款式 lookup reads it, and the scene step names its
            // products after it.
            tshirtFile: colorway.white[0],
            key: colorwayKey(colorway.id, print.file)
          });
        }
      }
      ...
      ctx.log("第三步：T恤「" + plan.tshirt.name + "」选了 " + plan.tshirt.colorways.length + " 个款式 × " +
      ...
      const settled = await mapLimit(todo, localConcurrency(), async function (pair, index) {
        ...
        // The 款式's own photos, 白底图 first (a product shot is the primary evidence
        // of what the garment looks like), then 细节图 (how it is made), then 模特图
        // (how it drapes on a body — the one thing a flat-lay cannot show). The
        // provider appends the 印花 at the END, so the garment is what anchors the
        // composition.
        //
        // 尺码图 are deliberately never sent: a size table is a diagram, not
        // evidence about fabric.
        const refs = [];
        for (const file of pair.colorway.white.concat(pair.colorway.detail, pair.colorway.model)) {
          const bytes = await store.getFile(file).catch(function () { return null; });
          if (bytes) refs.push({ buffer: bytes.buffer, mimeType: bytes.mimeType });
        }
        ...
        const row = {
          ...
          // Which colour this shot is of. The 白底图 alone does not say — the file
          // name is a uuid — so the listing would otherwise have no way to group
          // the shots of one colour together.
          colorwayId: pair.colorway ? pair.colorway.id : null,
          colorwayName: pair.colorway ? pair.colorway.name : null,
    ...
    if (allTshirtRows.length > 0 && plan.prompts.scene.found && plan.scenes.available > 0) {
      ...
      const sceneCount = plan.settings.sceneCount;
      const sceneOutputs = plan.settings.sceneOutputs;
      const items = [];
      for (const row of allTshirtRows) {
        ...
        for (let sceneIndex = 0; sceneIndex < sceneCount; sceneIndex++) {
          for (let variant = 0; variant < sceneOutputs; variant++) {
            items.push({
              tshirtFile: tshirtFile,
              sceneIndex: sceneIndex,
              variant: variant,
              key: sceneKey(tshirtFile, sceneIndex, variant),
              // Carried through so the product record can name what this 款式
              // actually is: which T恤 it is a colourway of, which T恤 photo was
              // used, and which 二创印花 was applied. The shelf shows those as the
              // listing's own fields (title block, "shop" line) instead of only
              // the group name — a 商品 with 24 款式 is 24 indistinguishable rows
              // otherwise.
              tshirtId: row.tshirtId || null,
              tshirtName: row.tshirtName || null,
              tshirtPhoto: row.tshirtFile || null,
              colorwayId: row.colorwayId || null,
              colorwayName: row.colorwayName || null,
              printId: row.printId || null,
              printFile: row.printFile || null
            });
          }
        }
      }
      ...
      ctx.log("第四步：换装+裂变，共 " + items.length + " 张，还需生成 " + todo.length + " 张（每件 " +
        sceneCount + " 张场景图 × 每张 " + sceneOutputs + " 张成片）");

      // Each 款式 is shot in `sceneCount` DIFFERENT scenes, and every shot of one
      // scene shares it — the sceneOutputs images of a scene are one "generation"
      // of that scene.
      //
      // The scenes are dealt from **one deck for the whole run**, not from a fresh
      // copy of the pool per 款式. Copying made "without replacement" true only
      // inside one 款式: two 款式 could draw the same scene, and the effect over a
      // run was a handful of backgrounds reused over and over while most of the
      // pool went untouched. Measured on this machine's first two runs: one scene
      // used 14 times, another 13, and only 2 scenes of 228 used exactly once —
      // which is what "经常选到同一张" looks like from the outside.
      //
      // The deck also starts with the scenes this group has already been shot in
      // removed, so a resumed run carries on into fresh backgrounds instead of
      // re-dealing the ones it began with.
      const alreadyUsed = new Set((plan.existing.products || []).map(function (p) { return p.sceneId; }));
      let deck = [];
      function dealScene() {
        if (deck.length === 0) {
          // Fisher–Yates, so a reshuffle (the pool is exhausted) is uniform rather
          // than biased the way repeated splice-drawing is.
          deck = scenes.filter(function (s) { return !alreadyUsed.has(s.id); });
          if (deck.length === 0) deck = scenes.slice();
      ...
      function scenesForTshirt(tshirtFile) {
        if (scenesFor.has(tshirtFile)) return scenesFor.get(tshirtFile);
        const chosen = [];
        for (let i = 0; i < sceneCount; i++) {
          const scene = dealScene();
          if (scene === undefined) break;
          chosen.push(scene);
        }
        scenesFor.set(tshirtFile, chosen);
        return chosen;
      }
      const planned = todo.map(function (item) {
        const chosen = scenesForTshirt(item.tshirtFile);
        return Object.assign({}, item, { scene: chosen[item.sceneIndex] || null });
      }).filter(function (item) { return item.scene !== null; });
      ...
      const settled = await mapLimit(planned, localConcurrency(), async function (item, index) {
        ...
        const outputs = await ctx.provider.generate({
          ...
          // 3:4, which is both what the scene pool holds and what the prompt asks
          // for ("3:4竖版"). Without it the service answers 1:1 and the framing the
          // model planned is thrown away — the reply is a square composition, not a
          // crop of a vertical one.
          size: "3:4",
          count: 1
        });
        const output = outputs[0];
        const file = await store.putFile(output.buffer, output.mimeType);
        const size = readImageSize(output.buffer);
        const product = {
          id: recordId(),
          workflowId: WORKFLOW_ID,
          groupKey: plan.group.key,
          ...
          kind: "scene",
          file: file,
          // Measured from the bytes just stored, so the waterfall can reserve
          // this tile's height instead of jumping as it loads.
          width: imageDimension(size && size.width),
          height: imageDimension(size && size.height),
          tshirtFile: item.tshirtFile,
          // The 款式 is "a T恤 photo × a 二创印花"; naming both, and the T恤 it
          // belongs to, is what lets the shelf render a real listing rather than
          // a wall of anonymous images. Null on records written before this, so
          // every consumer must treat them as optional.
          tshirtId: item.tshirtId || null,
          tshirtName: item.tshirtName || null,
          tshirtPhoto: item.tshirtPhoto || null,
          // Which colour this finished shot is of, carried all the way from the
          // 款式 so the shelf can group a colour's shots without re-deriving it
          // from the T恤 record (which may have been edited since).
          colorwayId: item.colorwayId || null,
          colorwayName: item.colorwayName || null,
          printId: item.printId || null,
          printFile: item.printFile || null,
