...
/**
 * Normalize one T恤 record to the 款式 (colorway) model.
 *
 * A T恤 is a product with one 款式 per colour. Each 款式 owns its 白底图 (one or
 * more: front, back, …) and its 细节图 (collar, cuff, fabric); 尺码图 hang off the
 * product, because one pattern means one size chart.
 *
 * Old records were flat — `{ name, images: [file, …] }`, one photo per 款式. They
 * are read as one 款式 per photo with that photo as its only 白底图. That reading
 * is lossless and matches what the pipeline already did with them, so a group that
 * ran before this change resolves to the same set of shots after it.
 *
 * `images` is kept as a **derived** field: every 白底图 across every 款式, in
 * order. It is what the pipeline composites and what 二创 picks from, and it lets
 * every reader that predates 款式 keep working unchanged. Writers must not append
 * to it — it is recomputed here on every read, so an edit made to the derived
 * array alone would be silently dropped.
 *
 * Module-level, not inside `createStore`, because `printPipeline` normalizes the
 * T恤 it is handed as well: it is driven by hand-built states in tests, and a
 * record that predates 款式 has to resolve to the same shots there as it does
 * after a store read.
 *
 * Done on read rather than as a one-shot migration because `state.json` is also
 * edited by hand and restored from backups; a record that predates the model must
 * not become unreadable because it was put back.
 */
function normalizeTshirt(tshirt) {
  if (!tshirt || typeof tshirt !== "object") return tshirt;
  const isFile = function (f) { return typeof f === "string" && f !== ""; };
  const tshirtId = typeof tshirt.id === "string" ? tshirt.id : "t";
  let colorways;
  if (Array.isArray(tshirt.colorways)) {
    colorways = tshirt.colorways.map(function (c, index) {
      const source = c && typeof c === "object" ? c : {};
      return {
        id: typeof source.id === "string" && source.id !== "" ? source.id : "cw-" + index + "-" + tshirtId,
        name: typeof source.name === "string" && source.name !== "" ? source.name : "款式 " + (index + 1),
        white: Array.isArray(source.white) ? source.white.filter(isFile) : [],
        detail: Array.isArray(source.detail) ? source.detail.filter(isFile) : [],
        // A shot of this colour ON a body. Deliberately not part of `images`: a
        // 白底图 is a composite target and a 模特上身图 is a listing asset, so
        // folding them together would make the pipeline render one product per
        // model shot.
        model: Array.isArray(source.model) ? source.model.filter(isFile) : []
      };
    });
  } else {
    const images = Array.isArray(tshirt.images) ? tshirt.images.filter(isFile) : [];
    // A legacy 款式 is identified by the photo it was built from, not by its
    // position: a group may have stored that id as its selection, and positions
    // shift as soon as a photo is added or removed.
    colorways = images.map(function (file, index) {
  ...
  return Object.assign({}, tshirt, {
    colorways: colorways,
    sizeImages: Array.isArray(tshirt.sizeImages) ? tshirt.sizeImages.filter(isFile) : [],
    images: images
  });
}
  ...
  async function readState() {
    try {
      ...
      return {
        ...
        tshirts: (Array.isArray(parsed.tshirts) ? parsed.tshirts : []).map(normalizeTshirt),
...
module.exports = { createStore, defaultRoot, normalizeTshirt };
