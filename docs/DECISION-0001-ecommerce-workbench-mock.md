...
当前模块清单（行为细节以 README 为准，此处只记录范围与归类）：主分组为 印花提取／印花二创／T恤二创／工作流；分割线下方依次为 通用工作台、T恤管理、提示词管理、场景图管理；印花二创另有「导入文件夹」用于批量导入工作台外已产出的印花。工作流另见 DECISION-0002。其余模块（工作台首页 / 模特管理 / 产品管理）不在当前阶段范围，后续版本开放。
